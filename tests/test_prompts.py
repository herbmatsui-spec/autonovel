...
class TestAPCandDeltaPolish(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_apc_system_prompt_determinism(self):
        ...
        settings_ctx_json = json.dumps(
            {"theme": "Dark Fantasy", "rules": [1, 2, 3]}, sort_keys=True
        )
        ...
        prompt1 = await self.pm.build_apc_system_prompt(
            style_key, write_rule_type, settings_ctx_json, hooks_inst, char_static_ctx
        )
        prompt2 = await self.pm.build_apc_system_prompt(
            style_key, write_rule_type, settings_ctx_json, hooks_inst, char_static_ctx
        )
    ...
    async def test_delta_polish_prompt(self):
        ...
        prompt = await self.pm.build_delta_polish_prompt(
            target_beat, target_word_count, prefix_text, suffix_text, instructions
        )
