...
async def test_narrative_logic():
    ...
    graph.add_node(
        NarrativeStateNode(
            NarrativeState.DAILY, "日常", "日常描写", [], "穏やか", ["いきなりの結末"]
        )
    )
    graph.add_node(
        NarrativeStateNode(
            NarrativeState.INCIDENT, "事件", "転機", ["日常の確立"], "衝撃", ["即座の解決"]
        )
    )
    graph.add_node(
        NarrativeStateNode(
            NarrativeState.CONFLICT, "葛藤", "深化", ["事件発生"], "緊張", ["唐突な和解"]
        )
    )
    graph.add_node(
        NarrativeStateNode(
            NarrativeState.CLIMAX, "絶頂", "決着", ["葛藤の蓄積"], "激昂", ["都合の良い救済"]
        )
    )
    graph.add_node(
        NarrativeStateNode(NarrativeState.RESOLUTION, "解決", "後日談", ["完了"], "安堵", [])
    )
    ...
    async def mock_dialogue_exec(*args, **kwargs):
        return MagicMock()
    ...
    async def mock_memory_exec(*args, **kwargs):
        return None
    ...
    async def mock_resonance_exec(*args, **kwargs):
        return None
    ...
    for i in range(3):
        ...
        await pipeline.process_scene_feedback(
            "CharA",
            "CharB",
            ConnectionState(affection=50, trust=50, tension=20),
            ["シーンの出来事"],
            {"affection": 1.0},
            context,
        )
        print(
            f"Scene {i + 1}: Current State = {context.narrative_state.name}, Progress = {context.global_state['narrative_progress']:.1f}"
        )
    ...
    await pipeline.process_scene_feedback(
        "CharA",
        "CharB",
        ConnectionState(affection=50, trust=50, tension=50),
        ["衝撃的な展開"],
        {"tension": 10.0},
        context,
    )
    ...
    await pipeline.process_scene_feedback(
        "CharA",
        "CharB",
        ConnectionState(affection=50, trust=50, tension=90),
        ["決戦中"],
        {"tension": 1.0},
        context,
    )
    print(
        f"After 1 scene in CLIMAX: Current State = {context.narrative_state.name} (Expected: CLIMAX)"
    )
