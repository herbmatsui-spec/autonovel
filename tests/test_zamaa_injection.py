...
async def test_zamaa_prompt_injection():
    ...
    logger.info(
        f"Testing tension_threshold correction: Expected 80, Got {config.tension_threshold}"
    )
    assert config.tension_threshold == 80, (
        "Tension threshold should be corrected to 80 for zamaa engine"
    )
