...
async def test_llm_service_generate_text_success():
    ...
    # 3. 実行
    request = LLMRequestOptions(prompt="Test prompt", model_name="gemini-1.5-pro", temp=0.7)
...
async def test_llm_service_generate_json_success():
    ...
    request = LLMRequestOptions(
        prompt="Generate JSON", model_name="gemini-1.5-pro", temp=0.0, response_schema=DummySchema
    )
