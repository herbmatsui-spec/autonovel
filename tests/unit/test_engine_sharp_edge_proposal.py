...
class TestParseSharpEdges:
    def test_key_phrase_handling(self):
        ...
        data = [
            {"edge_type": "ending_pullback", "description": "結末の説明", "key_phrase": "あ" * 21}
        ]
        raw = json.dumps(data)
        result = _parse_sharp_edges(raw)
        ...
        data = [{"edge_type": "ending_pullback", "description": "結末の説明"}]
        raw = json.dumps(data)
        result = _parse_sharp_edges(raw)
        ...
        data = [
            {"edge_type": "invalid_type", "description": "無効", "key_phrase": "無効なフレーズ"}
        ]
        raw = json.dumps(data)
        result = _parse_sharp_edges(raw)
        assert result == []
