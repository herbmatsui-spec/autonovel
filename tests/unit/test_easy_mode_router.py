...
@pytest.mark.asyncio
async def test_generate_content_success(monkeypatch, dummy_session):
    ...
    response = await easy_mode.generate_content(valid_input, session=dummy_session)
...
@pytest.mark.asyncio
async def test_generate_content_validation_error_path(monkeypatch, dummy_session):
    # Patch ValidationError used in the module with a dummy subclass
    ...
    with pytest.raises(ValidationException):
        await easy_mode.generate_content(valid_input, session=dummy_session)

@pytest.mark.asyncio
async def test_generate_content_service_exception(monkeypatch, dummy_session):
    # Force a generic exception inside the endpoint
    def raise_runtime(x):
