...
class TestCritiqueEdgeRejection:
    ...
    async def test_edge_loss_rejected(self):
        ...
        repo.get_all_non_anchor_chapters = AsyncMock(
            return_value=[MagicMock(ep_num=1, content="after content without edge")]
        )
        repo.get_all_plots = AsyncMock(
            return_value=[
                MagicMock(
                    ep_num=1,
        ...
        generate_json = AsyncMock(
            return_value=MagicMock(success=True, metadata={"prompt_patch": "patch"})
        )
    ...
    async def test_edge_preserved_passes(self):
        ...
        repo.get_book = AsyncMock(return_value=MagicMock(title="T", genre="test"))
        repo.get_all_non_anchor_chapters = AsyncMock(
            return_value=[MagicMock(ep_num=1, content="before after 余韻のある終わり方")]
        )
        repo.get_all_plots = AsyncMock(
            return_value=[
                MagicMock(
