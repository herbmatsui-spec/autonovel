...
async def test_generate_critic_feedback_success(prompt_manager):
    ...
    auditor = LogicalAuditor(repo=MagicMock(), pm=prompt_manager, llm=mock_llm, ctx_mgr=MagicMock())

    issue_list = LogicalAuditIssueList(
        is_consistent=False,
        issues=[AuditIssue(category="場所", severity="Major", description="突然の移動")],
        overall_severity="Major",
    )

    fb = await auditor.generate_critic_feedback(
        issue_list=issue_list, draft_content="本文", blueprint="設計図"
    )
...
async def test_generate_critic_feedback_fallback(prompt_manager):
    ...
    class MockRes:
        def __init__(self):
            self.success = False
            self.metadata = {"has_critical_issues": True, "rewrite_guidance": "突然の移動"}
