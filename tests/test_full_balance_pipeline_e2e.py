...
class TestFullBalancePipelineE2e:
    ...
    def test_phase2_hook_in_prompt(self):
        ...
        hook = EmotionalHookSpec(
            hook_name="catharsis", one_line_intent="解放", target_tension_peak=85
        )
        text = EMOTIONAL_HOOK_TEMPLATE.format(
            one_line_intent=hook.one_line_intent, target_tension_peak=hook.target_tension_peak
        )
        assert "解放" in text
    ...
    def test_phase4_sharp_edges_proposal(self):
        edges = [
            ...
            SharpEdgeSpec(
                edge_type="abnormal_dialogue", description="『退屈だね』という異常なセリフ"
            ),
        ]
    ...
    def test_phase5_edge_loss_rejection(self):
        ...
        result = asyncio.run(
            auditor.audit("after without edge", before_content="before", edges=edges)
        )
    ...
    async def test_phase7_gate_blocks_low_interest(self):
        ...
        checker.check = AsyncMock(
            return_value=EntertainmentCheckResult(
                interest_score=40,
