...
async def test_plot_expansion_workflow_success(harness):
    ...
    result = await workflow.execute(
        reporter=reporter, book_id=book_id, gen_from=gen_from, gen_to=gen_to, mode="final"
    )
...
async def test_plot_expansion_workflow_tension_warning(harness):
    ...
    mock_planner = AsyncMock()
    mock_planner.expand_plots.return_value = [MagicMock(ep_num=1, tension=80)]
...
async def test_plot_expansion_workflow_candidates_mode(harness):
    ...
    mock_planner = AsyncMock()
    mock_planner.expand_plots.return_value = [MagicMock(ep_num=1, tension=50)]
    ...
    await workflow.execute(
        reporter=None, book_id=book_id, gen_from=gen_from, gen_to=gen_to, mode="candidates"
    )
