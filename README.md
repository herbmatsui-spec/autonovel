...
- [2. 主要機能一覧 & 2大制作モード](#2-主要機能一覧--2大制作モード)
  ...
  - [2.3 アドバンスド・プロット & 世界観オーケストレーション](#23-アドバンスドプロット--世界観オーケストレーション)
...
```
[ジャンル・主人公・冒頭入力] ──> [AI執筆開始] ──> [非同期生成 (Huey)] ──> [リアルタイム進捗]
                                                                        │
[納品パッケージZIPダウンロード] <── [次話展開サジェスト] <── [本文プレビュー表示] <──┘
```

- **3案企画ガチャ (`POST /easy_mode/gacha`)**:
  漠然としたキーワードやジャンルから、AIが独自性のある3つの異なる企画アイデア（タイトル・あらすじ・主人公設定）を即座に提案。
- **インテリジェント・ダイジェスト生成 (`digest_service.py`)**:
  前話までの膨大なテキストから重要な文脈・未解決イベントを自動抽出し、トークン数を圧縮して次話プロンプトへ注入。
- **次話展開サジェスチョン Chips**:
  生成完了後、物語を盛り上げる「3つの次話展開案」をAIが自動提案。ワンクリックで次のプロンプトとして反映可能。
- **上級者モードへのワンクリック昇格 (`POST /easy_mode/promote`)**:
  かんたんモードで執筆した作品を、世界観Bible・キャラクターシート・プロットツリーを持つアドバンスドプロジェクトへ一瞬で変換。

---

### 2.3 アドバンスド・プロット & 世界観オーケストレーション

長編Web小説のヒットの法則をアルゴリズム化したプロ向けの本格制作システムです。

- **テンション曲線 & カタルシス計算エンジン (`Plot` モデル)**:
  各話の目標テンション値（0.0〜1.0）や、カタルシスの種類（ざまぁ、覚醒、正体バレ、ギャップ萌え等）、読者への感情フック（Next Hook）を数値的にシミュレーション。
- **マルチブランチ・ツリー管理 (`Branch` モデル)**:
  「もしあの時別の選択肢を選んでいたら？」というIFルート分岐をGitブランチのように木構造で作成・管理可能。
- **動的プロットリビルド (`PlotAgent` & `plot_rebuild_helpers.py`)**:
  執筆途中で設定変更や読者アンケート結果を取り入れた場合、以降の未執筆プロットを整合性を保ちながら自動再計算。
...
- **インタラクティブ・グラフ可視化 (`GraphVisualization.tsx`)**:
  フロントエンド上でキャラクター相関図や世界観エンティティの接続関係を物理シミュレーション付きネットワーク図として視覚化。
...
- **ワンクリック ZIP パッケージ出力 (`GET /easy_mode/export/{book_id}`)**:
