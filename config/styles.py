...
FORBIDDEN_SUMMARY_PATTERNS: Dict[str, str] = {
    "〜することになった": "【禁止】決定事項としての要約を禁ずる。結論を先に述べず、そこに至るまでの【動作 → 反応 → 変化】を時系列で具体的に描写せよ。",
    "〜という展開になった": "【禁止】メタ的なプロット進行の提示を禁ずる。物語の進行を説明せず、キャラの視界に映る光景と、身体が感じた衝撃のみを記述せよ。",
    "〜という結果に至った": "【禁止】結論への跳躍を禁ずる。結果だけを提示せず、因果関係を物理的な現象として積み上げ、読者に結果を悟らせよ。",
    "〜と考えたようだ": "【禁止】外部からの推測や視点漏れを禁ずる。キャラの脳内を説明せず、視線の動き、口元の歪み、呼吸の乱れなどの外形的徴候で表現せよ。",
}
...
STYLE_DEFINITIONS["style_fantasy_erotic"] = {
    "name": "幻想官能（ファンタジー風）",
    "instruction": "【指針】異種族の身体感覚と魔法の触れ合いを描き、官能とファンタジーを融合せよ。触覚・匂い・視覚・聴覚を豊かに表現し、《光・影・風》の比喩で極上感を高める。",
    "dialogue_ratio": "20%",
    "syntax_rhythm": "微細な感覚描写を短文で連続し、モンスターや魔法の動きと絡める。バッテリー感覚の瞬間を細部に落とす。",
    "metaphor_dna": "エメラルドの髪、流星の残光、蒸気の波紋。身体を光の湧き出る波として比喩。",
...
STYLE_DEFINITIONS["style_erotic_standard"] = {
    ...
    "dialogue_ratio": "30%",
    "syntax_rhythm": "緩急のある呼吸感。溜めの描写を長く取り、解放の瞬間を鮮烈に。文末の余韻を大切にせよ。",
    "metaphor_dna": "陶磁の肌、逆向きの波、溶け合う境界。身体を風景や現象に仮託する比喩。",
    "noise_dna": "乱れる呼吸、衣擦れの音、微かな汗の匂い。静寂の中の鼓動。",
    "golden_rules": RULE_SET_C_RULES,
    "negative_prompt": "AV的な機械的描写, 直接的な生殖器名称の連発, 感情を伴わない物理的動作の羅列",
}
...
def get_rule_set(rule_type: str) -> str:
    if rule_type == "RULE_SET_A":
        return RULE_SET_A_RULES
    elif rule_type == "RULE_SET_B":
        return RULE_SET_B_RULES
    elif rule_type == "RULE_SET_C":
        return RULE_SET_C_RULES
    elif rule_type == "RULE_SET_D":
        return RULE_SET_D_RULES
    return RULE_SET_A_RULES
