...
import { AskBibleResponse, ConsistencyIssue, DiffChunk } from "../../types/editor";
...
export const EditorialSidebar: React.FC<EditorialSidebarProps> = ({
   ...
   const [pendingDiff, setPendingDiff] = useState<DiffChunk | null>(null);
   ...
   const handleAudit = async () => {
     ...
     try {
       ...
       // Step 23: Integrate audit results into lineScores
       const lines = currentText.split('\n');
       const newScores: Record<number, any> = {};

       res.issues.forEach((issue) => {
         if (!issue.conflicting_text) return;
         const lineIdx = lines.findIndex(line => line.includes(issue.conflicting_text));
         if (lineIdx !== -1) {
           newScores[lineIdx] = {
             line: lineIdx + 1,
             score: issue.severity === 'error' ? 20 : issue.severity === 'warning' ? 50 : 80,
       ...
       // Update context state (merging with existing if necessary, but usually audit replaces)
       setLineScores(Object.values(newScores));
   ...
   const handleApplyFix = (issue: ConsistencyIssue, idx: number) => {
     ...
     const start = currentChapterText.indexOf(issue.conflicting_text);
     if (start === -1) {
       onToast?.("該当文が現在の本文中に見つかりませんでした", "error");
       return;
     }
     const end = start + issue.conflicting_text.length;

     setPendingDiff({
       original: issue.conflicting_text,
   ...
   const confirmDiffApply = () => {
     ...
     const { original, replacement, start, end } = pendingDiff;
     applyDiff(start, end, replacement);
     setActiveHighlight(null);

     // 矛盾診断リストから該当するものを削除（簡易的にテキスト一致で判定）
     setAuditIssues((prev) => prev.filter(issue => issue.conflicting_text !== original));
