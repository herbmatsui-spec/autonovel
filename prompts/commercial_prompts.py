...
COMMERCIAL_ROLE_PROMPT_TEMPLATES: Dict[str, Dict[str, str]] = {
    "avatar_of_desire": {
        "name": "自己投影・願望充足役",
        "scene_generation": (
            "読者が{character}に感情移入し、自己投影できる場面を生成してください。\n"
            "■ 鍵となる要素:\n"
            "  1. {character}の行動を「俺だったらこうするのに」という視点で描写\n"
            "  2. 読者の願望（最強・最美・絶対的受容）を直接満たす場面\n"
            "  3. 普遍的な欲望に訴える価値基準（正義・絆・成長）\n"
            "  4. 短絡的な満足ではなく、目標への「過程」を重視し、投資心理（愛着・応援意欲）を育成\n"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
            "  - 読者が渇望する「理想の自己像」を体現\n"
    ...
    "hate_magnet": {
        "name": "憎悪集積型悪役",
        "scene_generation": (
            "読者が{character}を憎む感情が蓄積される場面を生成してください。\n"
            "■ 憎悪蓄積パターン:\n"
            "  1. 不条理な傲慢さ（自分より立場の低い者を理不尽に見下す）\n"
            "  2. 無神経なまでに残酷な発言（他者の努力や誠意を嘲笑）\n"
            "  3. 理不尽に強大な存在としての描写（主人公や味方の善行に報いない）\n"
            "  4. 特権階級的な横柄さ（悪事の証拠があっても処罰されない）\n"
            "■ 最終的なカタルシスに向け、憎悪を戦略的に蓄積"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
            "  - 明確な「悪役」としての存在意義\n"
            "  - 打倒時の読者の快感を最大化することを目標とする\n"
            "  - 類型の役柄との差別化を確保"
        ),
    },
    "unconditional_supporter": {
        "name": "絶対的肯定シェルター",
        "scene_generation": (
            "読者と{character}が共にいる安心感・無条件の受容を感じる場面を生成してください。\n"
            "■ シェルター効果パターン:\n"
            "  1. 何をしても受け入れてくれる温かさ\n"
            "  2. 失敗しても否定しない信頼感\n"
            "  3. ただそこに存在してくれる安心感\n"
            "  4. 言葉を交わさずとも伝わる「理解されている」感覚\n"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
    ...
    "contrast_engine": {
        ...
        "scene_generation": (
            ...
            "  3. 学業優秀 × 運動音痴\n"
            "  4. 冷酷な一面 × 子供のような素直さ\n"
            "■ 読者が「新事実発見！」と驚き、親近感を覚える展開"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
            "  - 単一なイメージではなく「重層的な魅力」を付与\n"
            "  - 予想外の側面を発見させ、常に新鮮な感動を提供\n"
            "  - 話題が広がり、読者の評価が高まる要素"
        ),
    },
    "unique_value": {
        ...
        "scene_generation": (
            ...
            "■ 承認達成パターン:\n"
            "  1. 誰も気づかなかった長所に光が当てられる\n"
            "  2. その人だからこそ成し得る実績の証明\n"
            "  3. 長年の努力が報われる瞬間\n"
            "  4. 「お前はやはり特別なんだ」と認知される"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
            "  - 読者自身が抱く承認欲求との共鳴\n"
            "  - 「特別な存在になりたい」という願望の投影先\n"
            "  - 努力が報われるプロセスへの共感"
        ),
    },
    "growth_investment": {
        "name": "成長可視化・投資心理喚起",
        "scene_generation": (
            "{character}の成長過程を追い、その成長が読者に「投資した甲斐があった」と感じさせる場面を生成してください。\n"
            "■ 成長可視化パターン:\n"
            "  1. 過去の自分と現在の自分との明確な差分\n"
            "  2. 当時は無理だった事が「今」は簡単\n"
            "  3. かつて自分を見下していた存在を、「今」は一笑に付す\n"
            "  4. 成長速度が加速していることの可視化\n"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
            "  - 「あの頃から見るとこんなにも育った」感を最大化\n"
            "  - 初期の頃から見守ってきた読者が大きな達成感を味わう快感\n"
            "  - 成長ストーリーへの持続的関心"
        ),
    },
    "destined_resonance": {
        "name": "運命的結びつき",
        "scene_generation": (
            "{character}と他者との運命的な結びつきが感じられる場面を生成してください。\n"
            "■ 運命パターン:\n"
            "  1. 出会いが偶然ではない暗示\n"
            "  2. 似た傷を持つ者同士の本能的な引き寄せ合い\n"
            "  3. 二人の対等な構図や対照性の暗示\n"
            "  4. 因縁が解けて自然と結びつく関係の再生\n"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
            "  - 「この二人は最終的に結ばれるべき」という期待感\n"
            "  - 読者の愛の成就願望の投影先\n"
            "  - 関係性の深まりへの持続的期待"
        ),
    },
    "information_hegemony": {
        "name": "情報支配・知的快感",
        "scene_generation": (
            "{character}が情報を支配し、読者に「私だけが知っている」快感を与える場面を生成してください。\n"
            "■ 情報支配パターン:\n"
            "  1. 読者だけが知る秘密（他のキャラクターは知らない）\n"
            "  2. 他のキャラクターの誤解を読者が優位な視点から把握できる瞬間\n"
            "  3. 伏線が回収され、「だからか！」と繋がる快感\n"
            "  4. 複雑な人間関係が整理され、見通しが良くなる"
        ),
        "character_building": (
            "■ {character}の商業的価値を構築:\n"
            "  - 情報的優越感の提供\n"
    ...
    "status_flip_trigger": {
        "name": "地位反転トリガー",
        "scene_generation": (
            "{character}を通じて社会的地位の反転を描き、「待ってました！」というカタルシスを与える場面を生成してください。\n"
            "■ ステータス反転パターン:\n"
            "  1. 低位からの出発 → 高位への到達\n"
...
PLEASURE_GRAPH_PROMPT_TEMPLATES: Dict[str, str] = {
    "catharsis_building": (
        ...
        "  2. OBSTACLE_ACCUMULATION: 障壁・苦難の積み重ね\n"
        "  3. DESPERATE_STRUGGLE: 追い込み・決断の場面\n"
        "  4. CATHARTIC_RELEASE: 解放・成就・逆襲の瞬間\n"
        "■ 報酬設計: 苦難が深いほど、カタルシスは大きくなる"
    ),
    "schadenfreude_building": (
        "【審判・滅びのプロンプト】\n"
        "■ 目的: 憎い敵が失脚・没落する快感の最大化\n"
        "■ 構成要素:\n"
        "  1. HATE_ACCUMULATION: 悪行の蓄積による憎悪増幅\n"
        "  2. ARROGANCE_PEAK: 傲慢さが頂点に達する場面\n"
        "  3. EVIDENCE_REVELATION: 証拠・真実の暴露\n"
        "  4. PUBLIC_FALL: 皆の前での権威失墜・制裁\n"
        "■ 感情的設計: 憎い敵が「失脚・没落する瞬間」に最大のカタルシス"
    ),
    "tension_release_building": (
        ...
        "■ パターン:\n"
        "  1. 緊張局面（対立・危機・障壁）\n"
        "  2. 息継ぎの瞬間（微かな希望・小さな前進）\n"
        "  3. 緩和局面（危機突破・関係修復・成長の証）\n"
        "  4. 新しい緊張の予告\n"
        "■ 報酬設計: 「一山越えるたびに小さなカタルシス」を積み重ねる"
    ),
    "superiority_building": (
        "【優越感・全能感のプロンプト】\n"
        "■ 目的: 読者が「主人公は正しい・強い・美しい」を追体験\n"
        "■ 構成要素:\n"
        "  1. IDOLIZATION_TARGET: 理想像の描写\n"
        "  2. COMPARISON_CONTEXT: 周囲との比較での優越性\n"
        "  3. RIVAL_DEFEAT: 競合他者の論破・完封\n"
        "  4. RECOGNITION_ACQUISITION: 他者からの認知・称賛\n"
        "■ 報酬設計: 理想の自己像の投影と、他者より優れた優越感"
    ),
    "intimacy_building": (
        "【親密感・連帯感のプロンプト】\n"
        "■ 目的: キャラクター同士の絆・読者の参加意識\n"
        "■ 構成要素:\n"
        "  1. SHARED_STRUGGLE: 共に困難を乗り越える経験\n"
        "  2. VULNERABLE_MOMENT: 互いに無防備になる親密場面\n"
        "  3. MUTUAL_RECOGNITION: 相互理解・感謝の表明\n"
...
def generate_pleasure_graph_prompt(
    ...
    return (
        ...
        f"■ 快感設計原則:\n"
        f"  - 現在のカタルシス投資: {story_progress:.0%} 完了\n"
...
MARKETING_PROMPT_TEMPLATES: Dict[str, str] = {
    "hook_generation": (
        "【フック・キャッチコピー生成】\n"
        "■ 目的: 読者を惹きつける「読みたくなる」最初の1行\n"
        "■ 原則:\n"
        "  1. 好奇心扇動: 物語の始まりで疑問を提示\n"
        "  2. 感情惹起: 「かわいい」「感動」「怖い」等の即時的感情\n"
        "  3. 謎かけ: 読者が答えを探したくなる構図\n"
        "  4. 衝突提示: 魅力的な対立の暗示\n"
    ),
    "tagline_generation": (
        "【タグライン・本作を表現する一言】\n"
        ...
        "  4. 期待違反: 「留年勇者 vs 魔王――世界を救う前に単位が足りない」\n"
    ),
    "synopsis_generation": (
        "【あらすじ・概要生成】\n"
        "■ 目的: 作品の魅力を短時間で伝達\n"
...
STYLE_DNA_PROMPT_TEMPLATES: Dict[str, str] = {
    ...
    "character_voice_extraction": (
        "【キャラクターボイス抽出】\n"
        "■ 目的: 各キャラクターの対話パターンを定義\n"
        "■ 抽出要素:\n"
        "  1. 話し方の特徴（敬語/タメ口/独特の語尾）\n"
        "  2. 心理描写の頻度・形態\n"
        "  3. 価値観・口癖の特定\n"
        "  4. 他のキャラクターとの対話時の変化\n"
    ),
...
SERIES_EXPENSION_PROMPTS: Dict[str, str] = {
    "arc_planning": (
        "【シリーズ展開・中長期計画】\n"
        "■ 目的: 第1部での成功後、継続して展開する物語の計画\n"
        "■ 構成要素:\n"
        "  1. MAIN_CHARACTER_GROWTH: 主人公の成長曲線の設計\n"
    ...
    "sequel_hooks": (
        ...
        "■ 目的: 読者を次作に引き寄せる要素の確保\n"
        "■ パターン:\n"
        "  1. UNRESOLVED_TENSION: 未解決の緊張関係の維持\n"
        "  2. NEW_THREAT_INTRO: 新たな脅威の示唆\n"
        "  3. CHARACTER_POTENTIAL: 未発揮のキャラクターの潜在能力\n"
    ...
    "filler_strategy": (
        ...
        "  2. WORLD_BUILDING: 世界観の細部の可視化\n"
        "  3. RELATIONSHIP_BUILDING: 関係性を試す小波乱\n"
        "  4. COMEDY_RELIEF: 息抜き・ユーモアの挿入\n"
    ),
}
...
AB_TEST_PROMPTS: Dict[str, str] = {
    "opening_variation": (
        "【A/Bテスト: 開始場面バリエーション】\n"
        "■ テスト対象: 物語の入り口\n"
        "■ パターンA: 謎の提示から開始（読者の好奇心を即座に刺激）\n"
        "■ パターンB: 衝突・事件から開始（読者の感情を即座に引き込む）\n"
        "■ パターンC: 日常シーンから開始（読者の親近感を確保）\n"
    ),
    "conflictr_resolution_variation": (
        "【A/Bテスト: 衝突解決バリエーション】\n"
        "■ テスト対象: 障壁への対処\n"
        "■ パターンA: 力技で解決（物理的な解決・成長の証）\n"
        "■ パターンB: 知略で解決（戦略・話術・機知）\n"
        "■ パターンC: 偶然による解決（運命的な出会い・幸運）\n"
    ),
    "romance_development_variation": (
        "【A/Bテスト: ロマンス展開バリエーション】\n"
        "■ テスト対象: 恋愛要素の進め方\n"
        "■ パターンA: 一騎打ち型（互いをライバルとして強く意識する関係）\n"
        "■ パターンB: 信頼構築型（共に困難を乗り越える中で自然に芽生える関係）\n"
