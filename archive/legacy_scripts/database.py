...
# ---------------------------------------------------------
# 互換性パッチ: Pydanticモデルのスキーマ変更 (stress -> tension) 吸収用
# ---------------------------------------------------------
if not hasattr(BookDbModel, "cumulative_stress"):
    setattr(
        BookDbModel,
        "cumulative_stress",
        property(lambda self: getattr(self, "cumulative_tension", 0)),
    )
if not hasattr(BookDbModel, "cumulative_tension"):
    setattr(
        BookDbModel,
        "cumulative_tension",
        property(lambda self: getattr(self, "cumulative_stress", 0)),
    )
...
def retry_with_logging(
    retries: int = 15, base_delay: float = 0.0, max_delay: float = 60.0
):  # max_delayを延長
    def decorator(func):
...
class DatabaseManager:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            with conn:
                ...
                # インデックス作成
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_plot_book_ep ON plot(book_id, ep_num);"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_chapters_book_ep ON chapters(book_id, ep_num);"
                )
    ...
    def _ensure_schema_version_sync(self, conn: sqlite3.Connection) -> None:
        ...
        for col, col_def in target_cols_plot.items():
            if col not in existing_cols:
                conn.execute(f"ALTER TABLE plot ADD COLUMN {col} {col_def};")

        target_cols_books = {
            "cumulative_tension": "INTEGER DEFAULT 0",
            "cumulative_qol": "INTEGER DEFAULT 0",
            "cumulative_cost": "REAL DEFAULT 0.0",
            "sanctuary_integrity": "INTEGER DEFAULT 100",
            "current_branch_id": "INTEGER DEFAULT 1",
        }
        cursor = conn.execute("PRAGMA table_info(books);")
        existing_cols_books = {row[1] for row in cursor.fetchall()}
        for col, col_def in target_cols_books.items():
            if col not in existing_cols_books:
...
class DataRepository:
    ...
    async def get_plot(self, book_id: int, ep_num: int) -> Optional[PlotDbModel]:
        row = await self.db.fetch_one(
            "SELECT * FROM plot WHERE book_id=? AND ep_num=?", (book_id, ep_num)
        )
        if not row:
            return None
    ...
    async def get_all_plots(self, book_id: int) -> List[PlotDbModel]:
        rows = await self.db.fetch_all(
            "SELECT * FROM plot WHERE book_id=? ORDER BY ep_num", (book_id,)
        )
    ...
    async def update_plot_status_stress_love(
        self, book_id: int, ep_num: int, stress: int, love_meter: int
    ) -> None:
        await self.db.execute(
    ...
    async def delete_plots_from(self, book_id: int, start_ep: int) -> None:
        await self.db.execute(
            "DELETE FROM plot WHERE book_id=? AND ep_num >= ?", (book_id, start_ep)
        )
    ...
    async def save_full_world_bible(self, bible: WorldBible, **kwargs) -> int:
        ...
        try:
            ...
            try:
                ...
                plot_data = [
                    (book_id, ep, f"第{ep}話 (TBD)", "計画中...", "planned", "Hate")
    ...
    async def get_chapter(self, book_id: int, ep_num: int) -> Optional[ChapterDbModel]:
        row = await self.db.fetch_one(
            "SELECT * FROM chapters WHERE book_id=? AND ep_num=?", (book_id, ep_num)
        )
        if not row:
            return None
    ...
    async def delete_chapter(self, book_id: int, ep_num: int) -> None:
        await self.db.execute(
            "DELETE FROM chapters WHERE book_id=? AND ep_num=?", (book_id, ep_num)
        )
    ...
    async def get_plots_before_limit_1(self, book_id: int, ep_num: int) -> Optional[PlotDbModel]:
        ...
        return (
            PlotDbModel(**self._parse_row(dict(rows[0]), ["scenes", "next_hook"])) if rows else None
        )
