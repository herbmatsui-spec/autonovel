...
async def test_backend():
    ...
    try:
        ...
        for t in titles:
            print(
                f"- {t.get('title')} (Viral: {t.get('viral_score')}%, Aura: {t.get('aura_type')})"
            )
        ...
        if titles:
            ...
            oracle_res = await engine.critique.predict_hegemony_success(
                genre, title, concept, custom_hook=custom_hook
            )
