...
class CharacterRegistry(BaseModel):
    ...
    role: Optional[str] = Field(
        default="", validation_alias=AliasChoices("role", "position", "job", "class")
    )
    ...
    surface_persona: Optional[str] = Field(
        default="", description="周囲からどう見られているか、演じている役割"
    )
    inner_conflict: Optional[str] = Field(
        default="", description="演じている自分と本当の望みの間の葛藤"
    )
    core_trauma: Optional[str] = Field(default="", description="過去のトラウマや原初の欠落")
    save_the_cat_event: Optional[str] = Field(
        default="", description="読者が共感する人間味ある善行"
    )
    first_person: Optional[str] = Field(
        default="私",
        description="一人称",
        validation_alias=AliasChoices("first_person", "fp", "i", "first"),
    )
    second_person: Optional[str] = Field(
        default="貴方",
        description="二人称",
        validation_alias=AliasChoices("second_person", "sp", "you", "second"),
    )
    suffix_style: Optional[str] = Field(
        default="",
        description="特徴的な語尾",
    ...
    tone: Optional[str] = Field(
        default="", validation_alias=AliasChoices("tone", "voice", "manner")
    )
    ...
    social_mask_vs_truth: Optional[str] = Field(
    ...
    def unwrap_metadata(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            str_fields = [
                ...
                "iron_constraint",
                "fate_link",
                "social_mask_vs_truth",
            ]
            for f in str_fields:
                if f not in data:
                    data[f] = data.get(
                        f, "私" if f == "first_person" else "貴方" if f == "second_person" else ""
                    )
        return data
...
try:
    ...
    print(
        f"Success! MC Profile name: '{res.mc_profile.name}', Iron constraint: '{res.mc_profile.iron_constraint}'"
    )
