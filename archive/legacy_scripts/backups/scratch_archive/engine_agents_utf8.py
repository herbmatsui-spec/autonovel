...
class InternalLogicValidator(BaseAgent):
    ...
    async def validate_misunderstanding_flow(
        ...
        prompt = self.engine.pm.build_misunderstanding_validation_prompt(content, gap_desc)
...
class MarketingAgent:
    ...
    async def analyze_style_dna(self, sample_text: str) -> Dict[str, Any]:
        ...
        res = await self.engine._generate_json(
            MODEL_PLANNING, prompt, temp=0.3, response_schema=StyleDNA
        )
        return (
            res.metadata
            if res.success
...
class PlanningAgent:
    ...
    async def create_hegemony_plan(
        ...
        world_res = await self.engine._generate_json(
        ...
        bible_prompt = self.engine.pm.build_bible_creation_prompt(
            WorldBibleCore,
            world_rules.model_dump_json(),
            concept or genre + "の覇権作品",
            target_eps,
        )
        bible_res = await self.engine._generate_json(
        ...
        rm_prompt = self.engine.pm.build_roadmap_prompt(
            bible_core.title, bible_core.synopsis, target_eps, RoadmapList
        )
        rm_res = await self.engine._generate_json(
            MODEL_PLANNING, rm_prompt, response_schema=RoadmapList
        )
            ...
            async with sem:
                if not force:
                    ...
                    if (
                        ...
                        and len(existing.detailed_blueprint) > 100
                    ):
                        return existing

                ep_info = next((r for r in roadmap if r.get("ep_num") == ep_num), {})
                past_plots = await self.engine.repo.get_plots_between(
                    book_id, max(1, ep_num - 3), ep_num - 1
                )
                ...
                try:
                    ...
                    res = await self.engine._generate_json(
                        MODEL_PLOT_EXPANSION, prompt, response_schema=PlotEpisode
                    )
                    if res.success:
                        ...
                        if reporter:
                            reporter.report(
                                f"📝 第{ep_num}話 プロット詳細化完了: {p_data.title}", "info"
                            )
                        return p_data
                    else:
    ...
    async def rebuild_hegemony_plot(
        ...
        prompt_outline = self.engine.pm.build_rebuild_plot_outline_prompt(
            book.title,
            start_ep,
            new_total_eps,
            book.synopsis,
        ...
        if bible:
            ...
            await self.engine.repo.create_bible(
                book_id,
                settings_dict,
    ...
    async def audit_producer_plan(
        self, genre: str, keywords: str, trend_memo: str
        ...
        res = await self.engine._generate_json(
            MODEL_PLANNING, prompt, response_schema=HegemonyAuditResult, temp=0.4
        )
        if res.success:
            try:
