...
async def test_cache():
    ...
    metadata, story, usage = await client.generate_json(
        model_name="gemini-1.5-flash", prompt="Test prompt", temp=0.7, use_cache=True, max_retries=1
    )
    ...
    assert mock_client.models.generate_content.call_count == 2, (
        "Third call with use_cache=False should bypass cache and invoke the API"
    )
    ...
    text1 = await client.generate_text(
        model_name="gemini-1.5-flash", prompt="Test prompt text", temp=0.7, use_cache=True
    )
