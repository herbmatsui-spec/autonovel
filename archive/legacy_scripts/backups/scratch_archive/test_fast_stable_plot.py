...
async def test_beat_sheet_audit():
    ...
    bs = BeatSheet(
        items=[
            BeatSheetItem(
                beat_number=1,
                description="主人公が新しい魔法を開発する。それは強大な威力を持つ。",
                character_focus=["主人公"],
            ),
            BeatSheetItem(
                beat_number=2,
                description="魔法の代償として魔力を全て消費し、一時的に気を失う。",
                character_focus=["主人公"],
            ),
        ],
        summary="新しい魔法の開発と代償",
    ...
    bs_violate = BeatSheet(
        items=[
            BeatSheetItem(
                beat_number=1,
                description="ノーコストで無限に魔法を乱射し、敵を撃退する。",
                character_focus=["主人公"],
            )
        ],
        summary="無双劇",
...
def test_screener_pick_best():
    ...
    # Mock variants
    mock_std = MagicMock()
    mock_std.detailed_blueprint = (
        "標準的な展開のプロット。3000文字以上にするための長いテキスト..." * 100
    )
    ...
    variants = {"Standard": mock_std, "Twist": mock_twist}
