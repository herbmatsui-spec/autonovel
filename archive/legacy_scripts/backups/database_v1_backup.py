...
def retry_with_logging(
    retries: int = 15, base_delay: float = 0.1, max_delay: float = 60.0
):  # max_delayを延長
    def decorator(func):
...
class DatabaseManager:
    ...
    def start(self, retry_count: int = 0) -> None:
        ...
        if retry_count > 5:
            logger.error(
                "Maximum database initialization retries exceeded. Database state is invalid."
            )
        ...
        try:
            ...
            with conn:
                ...
                # 他のテーブルも同様に作成
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS bible (id INTEGER PRIMARY KEY AUTOINCREMENT, book_id INTEGER, settings TEXT, revealed TEXT, version INTEGER DEFAULT 0, last_updated TEXT, FOREIGN KEY (book_id) REFERENCES books(id) ON DELETE CASCADE);"
                )
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS branches (id INTEGER PRIMARY KEY AUTOINCREMENT, book_id INTEGER, name TEXT, parent_id INTEGER, fork_ep_num INTEGER DEFAULT 0, created_at TEXT, FOREIGN KEY (book_id) REFERENCES books(id) ON DELETE CASCADE);"
                )
                ...
                # その他テーブル...
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS characters (id INTEGER PRIMARY KEY AUTOINCREMENT, book_id INTEGER, name TEXT, role TEXT, registry_data TEXT, FOREIGN KEY (book_id) REFERENCES books(id) ON DELETE CASCADE);"
                )
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS optimization_history (id INTEGER PRIMARY KEY AUTOINCREMENT, book_id INTEGER, report_json TEXT, created_at TEXT, FOREIGN KEY (book_id) REFERENCES books(id) ON DELETE CASCADE);"
                )
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS style_fragments (id INTEGER PRIMARY KEY AUTOINCREMENT, tag TEXT, content TEXT, embedding_json TEXT, origin TEXT, created_at TEXT);"
                )
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS custom_styles (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, instruction TEXT, score INTEGER, analysis TEXT, created_at TEXT);"
                )
                conn.execute("""
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_plot_branch_ep ON plot(branch_id, ep_num);"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_chapters_branch_ep ON chapters(branch_id, ep_num);"
                )
            ...
            if "malformed" in err_msg or "corrupt" in err_msg or "i/o error" in err_msg:
                    ...
                    for suffix in ["-wal", "-shm", "-journal"]:
                        ...
                        if Path(sidecar).exists():
                            try:
                                ...
                                logger.warning(
                                    f"Could not move {sidecar}: {side_e}\n{traceback.format_exc()}"
                                )
                except OSError as move_e:
                    logger.error(
                        f"Critical failure during DB recovery: {move_e}\n{traceback.format_exc()}"
                    )
                ...
                logger.warning(
                    f"Database locked or busy: {e}. Retrying initialization ({retry_count + 1}/5)..."
                )
                if conn:
                    try:
                        conn.close()
    ...
    def _ensure_schema_version_sync(self, conn: sqlite3.Connection) -> None:
        ...
        try:
            ...
            if "sanctuary_integrity" not in current_books_cols:
                conn.execute(
                    "ALTER TABLE books ADD COLUMN sanctuary_integrity INTEGER DEFAULT 100;"
                )
            ...
            for (book_id,) in books:
                cursor = conn.execute(
                    "SELECT id FROM branches WHERE book_id=? AND name='Main'", (book_id,)
                )
                if not cursor.fetchone():
                    ...
                    conn.execute(
                        "UPDATE books SET current_branch_id=? WHERE id=?", (new_branch_id, book_id)
                    )
                else:
                    cursor = conn.execute(
                        "SELECT id FROM branches WHERE book_id=? AND name='Main'", (book_id,)
                    ...
                    conn.execute(
                        "UPDATE books SET current_branch_id=? WHERE id=? AND current_branch_id IS NULL",
                        (b_id, book_id),
                    )
...
class DataRepository:
    ...
    async def get_all_books(self) -> List[BookDbModel]:
        ...
        return [
            BookDbModel(**self._parse_row(dict(r), ["style_dna", "marketing_data"])) for r in rows
        ]
    ...
    async def get_branches(self, book_id: int) -> List[BranchDbModel]:
        rows = await self.db.fetch_all(
            "SELECT * FROM branches WHERE book_id=? ORDER BY created_at", (book_id,)
        )
    ...
    async def update_book_current_branch(self, book_id: int, branch_id: int) -> None:
        await self.db.execute(
            "UPDATE books SET current_branch_id=? WHERE id=?", (branch_id, book_id)
        )
    ...
    async def get_plot(
        ...
        row = await self.db.fetch_one(
            "SELECT * FROM plot WHERE branch_id=? AND ep_num=?", (target_branch_id, ep_num)
        )
        if not row:
            return None
    ...
    async def get_all_plots(
        ...
        target_branch_id = branch_id if branch_id is not None else book_id_or_branch_id
        rows = await self.db.fetch_all(
            "SELECT * FROM plot WHERE branch_id=? ORDER BY ep_num", (target_branch_id,)
        )
        return [
            PlotDbModel(
                **self._parse_row(
                    dict(r), ["scenes", "next_hook", "healed_fields", "state_integrity_score"]
                )
            )
            for r in rows
        ]
    ...
    @retry_with_logging()
    async def update_plot_status_stress_love(
        self, branch_id: int, ep_num: int, stress: int, love_meter: int
    ) -> None:
        await self.db.execute(
    ...
    async def delete_plots_from(self, branch_id: int, start_ep: int) -> None:
        await self.db.execute(
            "DELETE FROM plot WHERE branch_id=? AND ep_num >= ?", (branch_id, start_ep)
        )
            ...
            try:
                ...
                if book_id:
                    ...
                    row = await conn.execute(
                        "SELECT marketing_data FROM books WHERE id=?", (book_id,)
                    )
                    ...
                    merged_mkt = {
                        ...
                        "ab_test_candidates": new_mkt.get(
                            "ab_test_candidates"
                        ),  # テスト案は最新を維持
                    }
                    ...
                    row_b = await conn.execute(
                        "SELECT current_branch_id FROM books WHERE id=?", (book_id,)
                    )
                ...
                chars = [
                    (
                        book_id,
                        bible.mc_profile.name,
                        "主人公",
                        json.dumps(bible.mc_profile.model_dump(), ensure_ascii=False),
                    )
                ]
                chars += [
                    (book_id, s.name, s.role, json.dumps(s.model_dump(), ensure_ascii=False))
                ...
                plot_data = [
                    (book_id, branch_id, ep, f"第{ep}話 (TBD)", "計画中...", "planned", "Hate")
    ...
    async def get_optimization_history(self, book_id: int) -> List[Dict[str, Any]]:
        rows = await self.db.fetch_all(
            "SELECT * FROM optimization_history WHERE book_id=? ORDER BY created_at DESC",
            (book_id,),
        )
    ...
    async def get_chapter(self, branch_id: int, ep_num: int) -> Optional[ChapterDbModel]:
        row = await self.db.fetch_one(
            "SELECT * FROM chapters WHERE branch_id=? AND ep_num=?", (branch_id, ep_num)
        )
        if not row:
            return None
        return ChapterDbModel(
            **self._parse_row(dict(row), ["world_state", "trinity_review_log", "summary"])
        )
    ...
    async def get_chapters_before(self, branch_id: int, ep_num: int) -> List[ChapterDbModel]:
        ...
        return [
            ChapterDbModel(
                **self._parse_row(dict(r), ["world_state", "trinity_review_log", "summary"])
            )
            for r in rows
        ]
    ...
    async def get_all_non_anchor_chapters(
        ...
        rows = await self.db.fetch_all(
            f"SELECT * FROM chapters WHERE branch_id=? ORDER BY {order_by}{limit_clause}",
            (target_branch_id,),
        )
        return [
            ChapterDbModel(
    ...
    async def delete_chapter(
        ...
        target_branch_id = branch_id if branch_id is not None else book_id_or_branch_id
        await self.db.execute(
            "DELETE FROM chapters WHERE branch_id=? AND ep_num=?", (target_branch_id, ep_num)
        )

    @retry_with_logging()
    async def update_chapter_content(self, branch_id: int, ep_num: int, content: str) -> None:
        await self.db.execute(
            "UPDATE chapters SET content=? WHERE branch_id=? AND ep_num=?",
            (content, branch_id, ep_num),
        )
    ...
    async def get_plots_before_limit_1(self, branch_id: int, ep_num: int) -> Optional[PlotDbModel]:
        ...
        return PlotDbModel(
            **self._parse_row(
                dict(rows[0]), ["scenes", "next_hook", "healed_fields", "state_integrity_score"]
            )
        )
    ...
    async def recalculate_book_stress(self, book_id: int, branch_id: int = 1) -> int:
        ...
        rows = await self.db.fetch_all(
            "SELECT stress_delta FROM chapters WHERE branch_id=?", (branch_id,)
        )
        total_stress = sum(row["stress_delta"] or 0 for row in rows)
        await self.db.execute(
            "UPDATE books SET cumulative_stress=? WHERE id=?", (total_stress, book_id)
        )
        return total_stress
    ...
    async def recalculate_book_comfort(self, book_id: int, branch_id: int = 1) -> Tuple[int, int]:
        ...
        rows = await self.db.fetch_all(
            "SELECT qol_delta FROM chapters WHERE branch_id=?", (branch_id,)
        )
        total_qol = sum(row["qol_delta"] or 0 for row in rows)
        # 聖域の堅牢性は最新のプロットから取得
        latest_plot = await self.db.fetch_one(
            "SELECT state_integrity_score FROM plot WHERE branch_id=? ORDER BY ep_num DESC LIMIT 1",
            (branch_id,),
        )
        integrity = latest_plot["state_integrity_score"] if latest_plot else 100

        await self.db.execute(
            "UPDATE books SET cumulative_qol=?, sanctuary_integrity=? WHERE id=?",
            (total_qol, integrity, book_id),
        )
    ...
    async def recalculate_book_cost(self, book_id: int, branch_id: int = 1) -> float:
        ...
        latest_plot = await self.db.fetch_one(
            "SELECT cost_score FROM plot WHERE branch_id=? AND status='expanded' ORDER BY ep_num DESC LIMIT 1",
            (branch_id,),
        )
        total_cost = latest_plot["cost_score"] if latest_plot else 0.0
        await self.db.execute(
            "UPDATE books SET cumulative_cost=? WHERE id=?", (total_cost, book_id)
        )
        return total_cost
    ...
    async def save_pending_patch(
        ...
        return await self.db.fetch_lastrowid(
            ...
            (
                book_id,
                patch_type,
                json.dumps(patch_content, ensure_ascii=False)
