...
class LogicalAuditor:
    ...
    def __init__(self, repo, pm, ctx_mgr, generate_json):
        self.repo = repo
        self.pm = pm
    ...
    async def audit_plot(
        self, book_id: int, ep_num: int, plot_bp: str, script: str
    ) -> LogicalAuditResult:
        # 改善案3: 履歴取得を ContextManager に一元化
        past_facts = await self.ctx_mgr.build_past_context(book_id, ep_num)
        prompt = self.pm.build_logical_audit_prompt(past_facts, plot_bp, script)
        res = await self.generate_json(
            MODEL_PLOT_EXPANSION, prompt, response_schema=LogicalAuditResult
        )
        if res.success:
            return LogicalAuditResult.model_validate(res.metadata)
    ...
    async def audit_foreshadowing_payoff(
        ...
        prompt = self.pm.build_foreshadowing_audit_prompt(f_map, content)
        res = await self.generate_json(
            MODEL_PLOT_EXPANSION, prompt, response_schema=ForeshadowingAudit
        )
        if res.success:
            return ForeshadowingAudit.model_validate(res.metadata)
        return ForeshadowingAudit(is_recovered=True)
    ...
    async def lightweight_audit_world_state(
        ...
        for char, state in current.character_states.items():
            ...
            if "死亡" in prev_state and "生存" in state:
                conflicts.append(
                    f"矛盾検知: {char} が前話で死亡していますが、今話で生存状態になっています。"
                )
        if conflicts:
            return LogicalAuditResult(
                is_consistent=False, conflict_points=conflicts, severity="Major"
            )
        return LogicalAuditResult(is_consistent=True)
...
class InternalLogicValidator:
    ...
    def __init__(self, pm, generate_json):
        self.pm = pm
        self.generate_json = generate_json
    ...
    async def validate_misunderstanding_flow(
        ...
        prompt = self.pm.build_misunderstanding_validation_prompt(content, gap_desc)
        res = await self.generate_json(MODEL_PLANNING, prompt, temp=0.2)
...
class PlotIntegrityMonitor:
    ...
    async def check_integrity(
        self, keywords: List[str], blueprint: str, content: str
        ...
        critical_missing = [
            m for m in missing if f"重要:{m}" in blueprint or f"必須:{m}" in blueprint
        ]
        if critical_missing:
            is_ok = False
...
class MarketingAgent:
    ...
    def __init__(self, repo, pm, generate_json):
        self.repo = repo
        self.pm = pm
        self.generate_json = generate_json
    ...
    async def generate_marketing_pack(
        ...
        prompt = self.pm.build_marketing_pack_prompt(book_title, synopsis, latest_ep)
        res = await self.generate_json(MODEL_PLANNING, prompt)
        return res.metadata if res.success else None

    async def generate_titles(self, genre: str, keywords: str) -> List[str]:
        prompt = self.pm.build_title_generation_prompt(genre, keywords)
        res = await self.generate_json(MODEL_PLANNING, prompt)
    ...
    async def analyze_style_dna(self, sample_text: str) -> Dict[str, Any]:
        prompt = self.pm.build_style_dna_analysis_prompt(sample_text)
        res = await self.generate_json(MODEL_PLANNING, prompt, temp=0.3, response_schema=StyleDNA)
        return (
            res.metadata
            if res.success
            else {"name": "分析失敗", "instruction": "", "score": 0, "analysis": ""}
        )
    ...
    async def create_export_package(self, book_id: int) -> Tuple[bytes, str]:
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
            ...
            for p in plots:
                ...
                if p.summary:
                    plot_text += f"  一行あらすじ：{p.summary}\n"
                plot_text += f"【設計図】\n{p.detailed_blueprint}\n"
                if p.next_hook:
                    hook_desc = p.next_hook
                    if isinstance(p.next_hook, dict):
                        hook_desc = p.next_hook.get("description", "")
                    plot_text += f"【次回への引き】\n{hook_desc}\n"
                plot_text += "\n" + "-" * 30 + "\n\n"
            ...
            z.writestr(
                "05_作品データダンプ.json", json.dumps(dump_data, ensure_ascii=False, indent=2)
            )
...
class PlanningAgent:
    ...
    def __init__(self, repo, pm, generate_json, narrative=None):
        self.repo = repo
        self.pm = pm
        self.generate_json = generate_json
    ...
    async def create_hegemony_plan(
        ...
        cost_severity: int,
        target_eps: int,
        initial_plot_limit: int,
        reporter=None,
    ) -> Tuple[int, WorldBible]:
        if reporter:
            reporter.report("🌍 世界観の深層生成を開始...", "info")

        world_prompt = self.pm.build_world_creation_prompt(genre, keywords, WorldRules)

        world_res = await self.generate_json(
            MODEL_PLANNING, world_prompt, response_schema=WorldRules, temp=0.8
        )
        if not world_res.success:
            logger.warning(
                f"世界観設定の生成に失敗しました。デフォルト設定を使用します: {world_res.error_message}"
            )
        ...
        mc_prompt = self.pm.build_mc_creation_prompt(
            world_rules.model_dump_json(), genre, keywords, concept
        )

        mc_res = await self.generate_json(MODEL_PLANNING, mc_prompt, temp=0.8)
        ...
        sub_prompt = self.pm.build_sub_char_creation_prompt(
            world_rules.model_dump_json(),
            json.dumps(mc_data, ensure_ascii=False),
            world_rules.causality_map,
            mc_name,
        )
        sub_res = await self.generate_json(MODEL_PLANNING, sub_prompt, temp=0.8)
        ...
        bible_prompt = self.pm.build_bible_creation_prompt(
            WorldBibleCore,
            world_rules.model_dump_json(),
            concept or genre + "の覇権作品",
            target_eps,
        )
        bible_res = await self.generate_json(
            MODEL_PLANNING, bible_prompt, response_schema=WorldBibleCore
        )
        ...
        if subs_data:
            try:
                bible_core.sub_characters = [
                    CharacterRegistry.model_validate(OutputSanitizer.normalize_metadata(s))
                    for s in subs_data[:5]
                ]
            except Exception:
        ...
        mkt_prompt = self.pm.build_marketing_ab_test_prompt(bible_core.concept)
        mkt_res = await self.generate_json(MODEL_PLANNING, mkt_prompt)
        ...
        rm_prompt = self.pm.build_roadmap_prompt(
            bible_core.title, bible_core.synopsis, target_eps, RoadmapList
        )
        rm_res = await self.generate_json(MODEL_PLANNING, rm_prompt, response_schema=RoadmapList)
    ...
    async def audit_bible_completeness(self, bible: WorldBible, reporter=None) -> bool:
        ...
        # 1. 基本情報のチェック
        if not bible.title or "無題" in bible.title:
            issues.append("作品タイトルが未設定です。")
        if len(bible.synopsis) < 200:
            issues.append("全体あらすじの書き込みが不足しています。")

        # 2. キャラクターのチェック
        mc = bible.mc_profile
        if not mc.name:
            issues.append("主人公の名前が設定されていません。")

        # registry_data の構造を模したチェック（CharacterRegistryオブジェクトとして評価）
        if not mc.expansion_hooks:
            issues.append(
                f"主人公 {mc.name} の『描写フック(Expansion Hooks)』が空です。執筆の質が低下する恐れがあります。"
            )
        if not mc.first_person:
            issues.append(f"主人公 {mc.name} の一人称が未設定です。")

        # 3. 世界観・ロードマップのチェック
        if not bible.world_settings.causality_map:
            issues.append(
                "世界の因果律（Causality Map）が空です。展開がご都合主義になるリスクがあります。"
            )
        ...
        if not issues:
            if reporter:
                reporter.report(
                    "✨ 企画書の健全性チェック合格。プロット詳細化へ移行します。", "info"
                )
            return True
        else:
            for issue in issues:
                ...
                if reporter:
                    reporter.report(issue, "warning")

            # 致命的な欠落（ロードマップなし）がある場合のみ中断
            if roadmap_len == 0:
                if reporter:
                    reporter.report("🚨 致命的な欠落があるため、生成を中断しました。", "error")
                return False
    ...
    async def expand_plots(
        ...
        from backend.workflows.plot_generation_pipeline import PlotGenerationPipeline

        pipeline = PlotGenerationPipeline(
            repo=self.repo,
            pm=self.pm,
            ctx_mgr=self.narrative.ctx_mgr,
            generate_json=self.generate_json,
            logic_validator=self.narrative.logic_validator,
            auditor=self.narrative.auditor,
    ...
    async def rebuild_hegemony_plot(
        ...
        past_chapters = await self.repo.get_chapters_before(branch_id, start_ep)
        ...
        if reporter:
            reporter.report(
                f"🔨 第{start_ep}話〜第{new_total_eps}話のアーク再構築を開始...", "info"
            )

        prompt_outline = self.pm.build_rebuild_plot_outline_prompt(
            book.title,
            start_ep,
            new_total_eps,
            book.synopsis,
        ...
        arc_res = await self.generate_json(MODEL_PLANNING, prompt_outline)
        ...
        if bible:
            ...
            kept_arcs = [
                a for a in old_arcs if isinstance(a, dict) and a.get("end_ep", 0) < start_ep
            ]
            ...
            await self.repo.create_bible(
                book_id,
                settings_dict,
                (bible.version or 0) + 1,
                datetime.datetime.now().isoformat(),
            )

        await self.repo.update_book_target_eps(book_id, new_total_eps)
        await self.repo.delete_plots_from(branch_id, start_ep)

        for ep in range(start_ep, new_total_eps + 1):
            await self.repo.create_or_replace_plot(
                book_id=book_id,
                ep_num=ep,
                branch_id=branch_id,
                thought_process="",
    ...
    ) -> Optional[HegemonyAuditResult]:
        """PlanningAgent内でAIプロデューサー診断を実行"""
        from config.domain_profile_manager import DomainProfileService

        # プラットフォームプロファイルを取得
        profile = DomainProfileService.get_platform_profile(platform)
        platform_desc = profile.get("description", "")
        platform_preferred_tropes = ", ".join(profile.get("preferred_tropes", []))
        platform_tone = profile.get("tone", "")

        prompt = self.pm.build_producer_audit_prompt(
            genre,
            keywords,
            trend_memo,
            sanctuary=sanctuary,
            ...
            platform_preferred_tropes=platform_preferred_tropes,
            platform_tone=platform_tone,
        )

        # originality_score (0-100) を LLMの temperature (0.1 - 1.0) に変換
        temp_val = 0.3 + (originality_score / 100.0) * 0.6
        temp_val = max(0.1, min(temp_val, 1.0))

        res = await self.generate_json(
            MODEL_PLANNING, prompt, response_schema=HegemonyAuditResult, temp=temp_val
        )
        if res.success:
            try:
...
class WritingAgent:
    ...
    async def generate_episodes(
        ...
        book = await self.repo.get_book(book_id)
        branch_id = book.current_branch_id if book and book.current_branch_id else 1
        char_db_models = await self.repo.get_all_characters(book_id)
        ...
        integrity_monitor = PlotIntegrityMonitor()
        logic_validator = InternalLogicValidator(self.pm, self.generate_json)

        from backend.engine_prompts import get_rule_set

        rule_set_content = get_rule_set(write_rule_type)
        style_inst = self.pm.get_style_instruction(style_key)

        for ep_num in range(start_ep, end_ep + 1):
            self.current_ep_num = ep_num
            plot = await self.repo.get_plot(book_id, ep_num, branch_id=branch_id)
            if not plot:
                continue

            prev_plot = (
                await self.repo.get_plot(book_id, ep_num - 1, branch_id=branch_id)
                if ep_num > 1
                else None
            )
            stress_before_ep = (
                prev_plot.stress
                if prev_plot and prev_plot.stress is not None
                else (book.cumulative_tension or 0)
            )

            stress_ctx = self.narrative.compute_stress_phase(
                ep_num, stress_before_ep, plot.is_catharsis, genre_str
            )
            phase_instruction = stress_ctx.get("instruction", "")

            current_stress = plot.stress if plot.stress is not None else stress_before_ep

            static_ctx, dynamic_ctx, prev_ctx = await self.ctx_mgr.get_optimal_context_split(
            ...
            villain_inst = self.pm.get_villain_instruction(genre_str)

            is_important_ep = plot.is_catharsis or ep_num == 1
            settings_ctx = json.dumps(bible_settings, ensure_ascii=False)

            prev_prose = ""
            if ep_num > 1:
                prev_ch = await self.repo.get_chapter(branch_id, ep_num - 1)
            ...
            if reporter:
                reporter.report(
                    f"👑 第{ep_num}話 [ストレス:{current_stress}]: 最新コンテキストで執筆開始",
                    "info",
                )

            # --- 改善案9: Style RAG による文体サンプル注入 ---
            style_rag = StyleRagManager(self.client, self.repo)
            ...
            cfg = get_config()
            optimized_patch = getattr(cfg, "optimized_prompt_patch", "")
            if optimized_patch:
            ...
            char_hooks_list = []
            for c in chars:
                hooks = getattr(c, "expansion_hooks", [])
                if hooks:
            ...
            fw_prompt = self.pm.build_final_writing_prompt(
                ep_num=ep_num,
                plot_data=p_dump,
            ...
            for attempt in range(max_retries):
                ...
                final_res = await self.generate_json(
                ...
                if is_easy_mode:
                    ...
                    final_content = self.formatter.format_for_kakuyomu(raw_content)
                    break
                ...
                if is_integrity_ok:
                    ...
                    if attempt == max_retries - 1:
                        final_content = self.formatter.format_for_kakuyomu(
                            raw_content
                        )  # 最終リトライ失敗
            ...
            if should_amplify or should_fix:
                ...
                amplify_prompt = self.pm.build_amplify_prompt(
                    final_content, current_target_word_count, fix_inst + hooks_inst
                )

                res_amp = await self.generate_json(MODEL_WRITING, amplify_prompt, temp=0.85)
                _, amp_content = res_amp.unwrap_or({}, final_content)
                final_content = self.formatter.format_for_kakuyomu(amp_content)
            ...
            f_audit = (
                await self.auditor.audit_foreshadowing_payoff(book_id, ep_num, final_content)
            ...
            if should_deep_audit:
                ...
                light_audit = await self.auditor.lightweight_audit_world_state(current_ws, prev_ws)
            ...
            stress_delta = current_stress - stress_before_ep
            # update_book_cumulative_stress はプロット段階で行われるためここでは実行しない
            ...
            await self.repo.create_chapter(
                book_id,
                ep_num,
                ...
                tension_delta=stress_delta,
                branch_id=branch_id,
            )
            await self.repo.update_plot_status_tension_love(
                branch_id, ep_num, current_stress, final_meta.get("love_delta", 0)
            )
    ...
    async def generate_episodes_pipeline(
        ...
        cfg = get_config()
        configured_concurrency = getattr(cfg, "max_concurrency", 0)  # 0 means auto
        write_sem_value = (
        ...
        async def plotter():
            ...
            existing_plots = await self.repo.get_plots_between(branch_id, start_ep, end_ep)
            chapters = await self.repo.get_all_non_anchor_chapters(book_id, branch_id=branch_id)
            ...
            # プロット生成自体を直列で一括実行（内部で Sequential に生成される）
            try:
                if plots_to_generate:
                    if reporter:
                        reporter.report(f"🗺️ プロット生成開始: {plots_to_generate}", "info")
                    p_res = await self.planner.expand_plots(
                        book_id, plots_to_generate, arcs, reporter=reporter, force=False
                    )
                    for p in p_res:
                        if isinstance(p, dict) and p.get("status") == "failed_plot_gen":
                            failed_plot_generations.append(p)
                            if reporter:
                                reporter.report(
                                    f"⚠️ プロット生成失敗: 第{p.get('ep_num')}話", "warning"
                                )
                        else:
                            await plot_queue.put(p)
        ...
        async def writer():
            while True:
                ...
                if isinstance(item, dict) and item.get("status") == "failed_plot_gen":
                    ...
                    if reporter:
                        reporter.report(
                            f"⚠️ 第{item['ep_num']}話の執筆をスキップ (プロット生成失敗)", "warning"
                        )
                    plot_queue.task_done()
                    continue
                    ...
                    if reporter:
                        reporter.report(f"🚨 パイプラインで予期せぬエラーが発生: {item}", "error")
                ...
                try:
                    ...
                    logger.error(f"Writer Error at ep {ep}: {e}")
                    failed_episodes.append(
                    ...
                    if is_easy_mode:
                        if reporter:
                            reporter.report(
                                f"⚠️ 第{ep}話の執筆中にエラーが発生しましたが、続行します。",
                                "warning",
                            )
                    else:
                        stop_event.set()
    ...
    async def analyze_and_import_chapter(
        ...
        try:
            cleaned_content = self.formatter.format_for_kakuyomu(content)
            book = await self.repo.get_book(book_id)
            if not book:
                return {"error": "作品が見つかりません"}
            branch_id = book.current_branch_id if book and book.current_branch_id else 1
            plot = await self.repo.get_plot(book_id, ep_num, branch_id=branch_id)
            prompt = self.pm.build_analyze_import_chapter_prompt(cleaned_content, EpisodeDraft)
            res = await self.generate_json(MODEL_PLANNING, prompt, response_schema=EpisodeDraft)
            if res.success:
                data = res.metadata
                # 保存処理
                await self.repo.create_chapter(
