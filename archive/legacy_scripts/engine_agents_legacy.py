...
class LogicalAuditor:
    ...
    async def audit_plot(self, book_id: int, ep_num: int, plot_bp: str, script: str) -> LogicalAuditResult:
        ...
        res = await self.generate_json(MODEL_PLOT_EXPANSION, prompt, response_schema=LogicalAuditResult)
    ...
    async def audit_foreshadowing_payoff(self, book_id: int, ep_num: int, content: str) -> ForeshadowingAudit:
        ...
        res = await self.generate_json(MODEL_PLOT_EXPANSION, prompt, response_schema=ForeshadowingAudit)
...
class InternalLogicValidator:
    ...
    async def validate_misunderstanding_flow(self, content: str, gap_desc: str) -> Tuple[bool, List[str]]:
        prompt = (
            self.pm.build_misunderstanding_validation_prompt(content, gap_desc)
        )
...
class PlanningAgent:
    ...
    async def create_hegemony_plan(
        ...
        world_res = await self.generate_json(MODEL_PLANNING, world_prompt, response_schema=WorldRules, temp=0.8)
        ...
        bible_prompt = self.pm.build_bible_creation_prompt(WorldBibleCore, world_rules.model_dump_json(), concept or genre + 'の覇権作品', target_eps)
        bible_res  = await self.generate_json(MODEL_PLANNING, bible_prompt, response_schema=WorldBibleCore)
        ...
        rm_prompt = self.pm.build_roadmap_prompt(bible_core.title, bible_core.synopsis, target_eps, RoadmapList)
    ...
    async def rebuild_hegemony_plot(
        ...
        prompt_outline = self.pm.build_rebuild_plot_outline_prompt(book.title, start_ep, new_total_eps, book.synopsis, keywords, trend_memo, pending_foreshadowing)
        ...
        if bible:
            ...
            await self.repo.create_bible(book_id, settings_dict, (bible.version or 0) + 1, datetime.datetime.now().isoformat())
        ...
        for ep in range(start_ep, new_total_eps + 1):
            await self.repo.create_or_replace_plot(
                book_id=book_id, ep_num=ep, branch_id=branch_id,
                thought_process="",
                title="【未生成 (TBD)】",
    ...
    async def audit_producer_plan(
        ...
        res = await self.generate_json(MODEL_PLANNING, prompt, response_schema=HegemonyAuditResult, temp=temp_val)
        if res.success:
            try:
...
class WritingAgent:
    """本文執筆とパイプライン管理を担当"""
    def __init__(self, repo, pm, ctx_mgr, generate_json, narrative, formatter, auditor, planner, client: Optional[Any] = None):
        self.repo = repo
        self.pm = pm
        self.ctx_mgr = ctx_mgr
        self.generate_json = generate_json
    ...
    async def generate_episodes(
        ...
        for ep_num in range(start_ep, end_ep + 1):
            ...
            prev_plot = await self.repo.get_plot(book_id, ep_num - 1, branch_id=branch_id) if ep_num > 1 else None
            stress_before_ep = prev_plot.stress if prev_plot and prev_plot.stress is not None else (book.cumulative_tension or 0)

            stress_ctx = self.narrative.compute_stress_phase(ep_num, stress_before_ep, plot.is_catharsis, genre_str)
            phase_instruction = stress_ctx.get("instruction", "")

            current_stress = plot.stress if plot.stress is not None else stress_before_ep

            static_ctx, dynamic_ctx, prev_ctx = await self.ctx_mgr.get_optimal_context_split(book_id, ep_num, plot, chars)
            ...
            if reporter:
                reporter.report(f"👑 第{ep_num}話 [ストレス:{current_stress}]: 最新コンテキストで執筆開始", "info")
            ...
            for attempt in range(max_retries):
                ...
                final_res = await self.generate_json(MODEL_WRITING, fw_prompt, system_instruction=sys_inst, temp=temp)
                ...
                if is_integrity_ok:
                    ...
                    if attempt == max_retries - 1:
                        final_content = self.formatter.format_for_kakuyomu(raw_content) # 最終リトライ失敗
            ...
            if should_amplify or should_fix:
                ...
                amplify_prompt = self.pm.build_amplify_prompt(final_content, current_target_word_count, fix_inst + hooks_inst)
            ...
            f_audit = await self.auditor.audit_foreshadowing_payoff(book_id, ep_num, final_content) if should_deep_audit else ForeshadowingAudit(is_recovered=True)
            audit_log_data = {}
        ...
        async def plotter():
            ...
            try:
                if plots_to_generate:
                    if reporter: reporter.report(f"🗺️ プロット生成開始: {plots_to_generate}", "info")
                    p_res = await self.planner.expand_plots(book_id, plots_to_generate, arcs, reporter=reporter, force=False)
                    for p in p_res:
                        if isinstance(p, dict) and p.get("status") == "failed_plot_gen":
                            failed_plot_generations.append(p)
                            if reporter: reporter.report(f"⚠️ プロット生成失敗: 第{p.get('ep_num')}話", "warning")
                        else:
