...
MODEL_CONFIG_DEFAULTS = {"populate_by_name": True, "extra": "allow", "protected_namespaces": ()}
...
class CharacterRegistry(BaseModel):
    ...
    suffix_style: str = Field(
        default="", description="特徴的な語尾（〜ですわ、〜だぜ、〜なのだ等）"
    )
    ability: str = Field(default="", alias="power")
    background: str = Field(default="", alias="bg")
    tone: str = Field(default="")
    iron_constraint: str = Field(
        default="", description="絶対に破らない行動原則・禁忌", alias="iron_const"
    )
    fate_link: str = Field(
        default="", description="世界の因果律との繋がり、世界が壊れた際に失うもの"
    )
    ...
    expansion_hooks: List[str] = Field(
        default_factory=list, alias="exp_hooks", description="描写を膨らませるための固有要素"
    )
        ...
        rels_str = _json.dumps(
            {
                ...
                for k, v in getattr(self, "relationships", {}).items()
            },
            ensure_ascii=False,
        )
...
class NarrativeConstraint(BaseModel):
    ...
    constraint: str = Field(
        ..., description="制約内容（〜を知らない、〜を持っている、〜できない等）"
    )
...
class Foreshadowing(BaseModel):
    ...
    description: str = Field(
        default="",
        description="伏線の具体的な内容（例：主人公が拾った錆びた鍵が実は王家の証である等、具体的に）",
    )
    exposure_level: int = Field(
        default=3, ge=1, le=5, description="読者へのバレやすさ(1:完全隠蔽〜5:露骨な予告)"
    )
...
class ClimaxScene(BaseModel):
    ...
    event: str = Field(
        default="", description="名場面の内容。何が起き、誰がどうなり、どのようなカタルシスがあるか"
    )
    target_emotion: str = Field(
        default="", description="読者に抱かせたい感情（例：絶望、鳥肌が立つような爽快感、感動の涙）"
    )
    pre_written_dialogue: str = Field(default="", description="決定的なセリフの応酬（魂の対話）")


class WorldRules(BaseModel):
    magic_cost_and_taboo: str = Field(
        default="なし", description="魔法やスキルの残酷な代償と世界の禁忌"
    )
    ...
    geopolitics_and_economy: str = Field(
        default="なし", description="資源の偏り、通貨の価値、地政学的緊張"
    )
...
class MarketingAssets(BaseModel):
    ...
    ab_test_candidates: List[Dict[str, Any]] = Field(
...
class SceneBeatBlock(BaseModel):
    ...
    action_description: str = Field(
        ..., description="具体的な行動描写。キャラが何をし、何が起きるか"
    )
...
class PlotEpisode(BaseModel):
    ...
    thought_process: str = Field(
        default="",
        description="プロット作成前の思考。Step A(矛盾検証), Step B(反証論破), Step C(最終結論)の形式で論理的に記述",
    )
    title: str = Field(default="", description="この話のサブタイトル")
    one_line_summary: str = Field(
        default="", description="この話の核心を突く一行あらすじ（読者の興味を惹くもの）"
    )
    detailed_blueprint: str = Field(
        default="",
        description="物語の設計図。起承転結の流れ、状況の変化、キャラの行動を詳細に記述",
        validation_alias=AliasChoices("detailed_blueprint", "blueprint", "plot", "summary"),
    )
    ...
    script_content: str = Field(
        default="",
        validation_alias=AliasChoices("script_content", "script", "manuscript", "content"),
    )
    ...
    lite_model_director_notes: str = Field(
        default="",
        description="生成したプロットの弱点と修正点",
        validation_alias=AliasChoices(
            "lite_model_director_notes", "self_critique", "director_notes"
        ),
    )
...
class RoadmapItem(BaseModel):
    ...
    resolution_style: Literal[
        "Cheat", "Logic", "Focus_Drama", "Self_Destruction", "Third_Party", "Misunderstanding_Peace"
    ] = Field(...)
...
class WorldBibleCore(BaseModel):
    ...
    subplots: List[SubplotThread] = Field(
        default_factory=list, description="独立したサブプロット管理"
    )
