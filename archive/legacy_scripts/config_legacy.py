...
NARRATIVE_PROPS: List[str] = [
    ...
    "香草が焦げるような、妙に癖になる匂い",
    "遠くで聞こえる、リズムの狂った鐘の音",
    "琥珀の中に閉じ込められた得体の知れない虫",
]

DAILY_MICRO_HOOKS: List[str] = [
    "靴の紐が何度も解けることへの苛立ち",
    "料理の塩加減が完璧だったことへの小さな恐怖",
    "鏡に映った自分の顔が、一瞬だけ他人のものに見える錯覚",
    "お気に入りの道具が、自分の意志を持っているかのように手に馴染む瞬間",
...
CHARACTER_EXPANSION_THEMES: Dict[str, List[str]] = {
    "主人公": [
        "過去の挫折・不当な扱いのフラッシュバック",
        "独自の技術・武器・魔術へのフェティッシュな執着",
        "内なる矛盾や独白",
    ],
    "ヒロイン": [
        "主人公への無自覚な独占欲や微細な嫉妬",
...
RULE_SET_C_NEG = "現代知識, 地球の技術, マヨネーズ, 上下水道, 記憶喪失, 思い出の消失, 名前を忘れる"
...
def get_config() -> GlobalConfigModel:
    ...
    if _global_config is None:
        _global_config = GlobalConfigModel.default()
...
class GlobalConfig:
    ...
    def display_sidebar(self) -> None:
        ...
        st.sidebar.number_input(
            "max_concurrency (0=Auto)",
            min_value=0,
            max_value=20,
            value=self.get("max_concurrency", 0),
            key="cfg_max_concurrency",
        )
        st.sidebar.checkbox("auto_backup", value=self.get("auto_backup", True), key="auto_backup")
