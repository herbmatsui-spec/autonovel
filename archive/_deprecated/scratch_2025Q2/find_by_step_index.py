        ...
        try:
            ...
            for tc_idx, tc in enumerate(tool_calls):
                ...
                if "engine_prompts.py" in args_str:
                    print(
                        f"Line {line_idx} (step_index={step_idx}) has tool call targeting engine_prompts.py: {tc.get('name')}"
                    )
                    ...
                    for key in ["CodeContent", "ReplacementContent"]:
                        if key in args:
                            ...
                            if "def build_drafting_prompt" in val:
                                print(
                                    f"  -> Found 'build_drafting_prompt' in {key} (length {len(val)})!"
                                )
                                out_name = (
                                    f"i:\\claude2\\scratch\\extracted_step_{step_idx}_{tc_idx}.py"
                                )
