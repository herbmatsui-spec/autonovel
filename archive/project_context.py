...
def get_config() -> Settings:
    """統一設定インスタンスを取得 (非推奨: `config.settings.get_settings()` を使用してください)"""
    warnings.warn(
        "config.project_context.get_config() は非推奨です."
        "代わりに config.settings.get_settings() を使用してください.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def set_config(config: Settings) -> None:
    """設定インスタンスを設定 (非推奨)"""
    warnings.warn(
        "config.project_context.set_config() は非推奨です."
        "設定の変更は環境変数または .env ファイルで行ってください.",
        DeprecationWarning,
        stacklevel=2,
    )
    global _settings_instance
...
class ProjectContext:
    """
    プロジェクト全体の設定に型安全にアクセスするためのシングルトン風ユーティリティ (非推奨).
    静的メソッドのみで構成し,インスタンス化不要.
    内部的には統一設定クラス (Settings) に委譲し,設定の一元管理を保証します.

    非推奨: `config.settings.get_settings()` または `config.settings.Settings` を直接使用してください.
    """

    @staticmethod
    def get_setting(key: str, default: Any = None) -> Any:
        """設定値を取得(型安全.デフォルト値対応) (非推奨)"""
        warnings.warn(
            "ProjectContext.get_setting() は非推奨です."
            "代わりに config.settings.get_settings().KEY を直接使用してください.",
            DeprecationWarning,
            stacklevel=2,
        )
        cfg = get_config()
    ...
    def reset_overrides() -> None:
        """テストや動的オーバーライドをリセットし,デフォルト値に戻す. (非推奨)"""
        warnings.warn(
            "ProjectContext.reset_overrides() は非推奨です."
            "代わりに config.settings.reset_settings() を使用してください.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def validate_consistency() -> list[str]:
        ...
        warnings.warn(
            ...
            "代わりに config.settings.get_settings().validate_consistency() を使用してください.",
            DeprecationWarning,
            stacklevel=2,
        )
...
class GlobalConfig:
    ...
    def __init__(self):
        warnings.warn(
            "GlobalConfig は非推奨です."
            "代わりに config.settings.get_settings() を直接使用してください.",
            DeprecationWarning,
            stacklevel=2,
        )
        pass
    ...
    def update(self, **kwargs) -> None:
        """複数の設定値を一括更新する(ランタイムのみ,永続化は別途必要) (非推奨)"""
        warnings.warn(
    ...
    def get_auto_concurrency(self) -> int:
        warnings.warn(
            ...
            "代わりに config.settings.get_settings().get_auto_concurrency() を使用してください.",
            DeprecationWarning,
            stacklevel=2,
        )
...
PROMPT_TEMPLATES: Dict[str, str] = {
    "style_instruction.j2": "[Target Style: {{ style_name }}]\n[Dialogue Ratio]: {{ dialogue_ratio }}{{ instruction }} {{ dna_correction }}",
}
