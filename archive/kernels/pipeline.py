...
class ConnectionPipeline:
    ...
    async def process_scene_feedback(
        ...
        # 感情変動を外部刺激として変換
        external_impact = {
            "resonance": emotional_shifts.get("affection", 0.0) * 0.5
            + emotional_shifts.get("trust", 0.0) * 0.5,
        ...
        # 次の状態を計算
        next_k_state = await self.interaction_mgr.compute_next_state(
            current_k_state, external_impact, context
        )
        ...
        if self.hegemony_eng:
            self.hegemony_eng.state_value = next_k_state.hegemony
        if self.conflict_eng:
            self.conflict_eng.state_value = next_k_state.conflict
        if self.serenity_eng:
            self.serenity_eng.state_value = next_k_state.serenity
        ...
        if self.narrative_mgr:
            ...
            context.global_state["description_density"] = (
                self.narrative_mgr.current_description_density
            )
            ...
            next_state = self.narrative_mgr.suggest_next_state(
                current_progress + 0.1, forced_state=forced_state, preferred_path=preferred_path
            )

            if next_state != self.narrative_mgr.current_state:
                print(
                    f"[Narrative] Transitioning state: {self.narrative_mgr.current_state} -> {next_state}"
                )
            ...
            if violations:
                print(
                    f"[Narrative Warning] State {self.narrative_mgr.current_state} violations detected: {violations}"
                )
                ...
                context.global_state["narrative_repair_count"] = (
                    context.global_state.get("narrative_repair_count", 0) + 1
                )
            else:
    ...
    async def prepare_next_scene_context(
        self,
        char_a: str,
        char_b: str,
        state: ConnectionState,
        ...
        if self.narrative_mgr:
            ...
            print(
                f"[Pipeline] Priority Kernels for {self.narrative_mgr.current_state}: {priority_kernels}"
            )
        ...
        res_prompt = (
            self.resonance_eng.generate_plot_injection_prompt(char_a, char_b, res_event)
            if res_event
            else ""
        )
