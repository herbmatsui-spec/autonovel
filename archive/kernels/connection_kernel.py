...
class ConnectionKernel(KernelBase):
    ...
    async def execute(self, context):
        ...
        if not trigger:
            ...
            if not getattr(analytics, "is_connection_event", False) and not is_priority:
                logger.info(
                    "Relationship is evolving naturally. No intervention needed",
                    kernel=self.persona["name"],
                )
                return None

        logger.info(
            "Trigger detected. Initiating Resonance Sequence",
            kernel=self.persona["name"],
    ...
    async def generate_resonance_event(self, context, trigger=None):
        ...
        if narrative_state == NarrativeState.DAILY and connection_state.resonance > 80:
            logger.info(
                f"[Narrative Trigger] High resonance ({connection_state.resonance}) detected in DAILY state. Suggesting transition to INCIDENT.",
                kernel=self.persona["name"],
            )
        ...
        try:
            ...
            prompt_text = await prompt_manager.render_async(
                "connection_resonance_prompt", **prompt_vars
            )
    ...
    def _select_connection_pattern(self, context, trigger) -> Dict[str, Any]:
        ...
        if not os.path.exists(path):
            return {
                "id": "generic",
                "name": "Generic Resonance",
                "description": "A subtle alignment of emotional states.",
            }
    ...
    async def apply_resonance_polish(self, scene_text: str) -> str:
        ...
        try:
            ...
            prompt_text = await prompt_manager.render_async(
                "resonance_polish_prompt", scene=scene_text
            )
