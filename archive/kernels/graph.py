...
class NarrativeStateGraph:
    ...
    def get_valid_transitions(
        self, current_state: NarrativeState, path: Optional[str] = None
    ) -> List[NarrativeState]:
        if path:
            return [
                to_state
                for (from_state, to_state), (weight, p) in self.edges.items()
                if from_state == current_state and p == path
            ]
        return [
            to_state for (from_state, to_state) in self.edges.keys() if from_state == current_state
        ]
...
class NarrativeStateManager:
    ...
    def __init__(
        self, graph: NarrativeStateGraph, initial_state: NarrativeState = NarrativeState.DAILY
    ):
        self.graph = graph
    ...
    def validate_transition(
        self, next_state: NarrativeState, path: Optional[str] = None
    ) -> Tuple[bool, str]:
        """
        ...
        return (
            False,
            f"Invalid transition from {self.current_state.name} to {next_state.name} (path: {path}). Valid options: {[s.name for s in valid_transitions]}",
        )
    ...
    def suggest_next_state(
        ...
        forced_state: Optional[NarrativeState] = None,
        ...
        valid_transitions = self.graph.get_valid_transitions(
            self.current_state, path=preferred_path
        )
        ...
        if target_phase == "climax":
            for (src, tgt), edge in self.edges.items():
                ...
                if edge.state.affection > 80 and edge.state.trust < 50:
                    suggestions.append(
                        f"{src}と{tgt}の間の不信感を爆発させ、共鳴イベントを誘発させてください。"
                    )
...
class PleasureGraph:
    ...
    def add_inhibition_link(self, source_id: str, target_id: str, strength: float = 0.5) -> bool:
        ...
        for edge in self.edges:
            if edge.target_node == node_id:
                ...
                if source_node and source_node.story_position <= current_position:
                    ...
                    boost += (
                        (source_node.intensity / 100.0)
                        * edge.causality_strength
                        * edge.amplification_factor
                        * time_factor
                    )
    ...
    def get_pleasure_graph_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "commercially_generated_nodes": sum(
                1 for n in self.nodes.values() if n.commercial_role_source
            ),
        }
    ...
    def validate_graph_balance(self) -> Dict[str, Any]:
        ...
        catharsis_nodes = [
            n for n in self.nodes.values() if n.pleasure_type == PleasureType.CATHARSIS
        ]
        ...
        # 後半の快感密度が低い
        late_pleasure = sum(n.intensity for n in self.nodes.values() if n.story_position > 0.7)
        early_pleasure = sum(n.intensity for n in self.nodes.values() if n.story_position < 0.3)
...
def build_default_pleasure_graph(
    story_arc_type: str = "exile_rise", target_length_ticks: int = 100
) -> PleasureGraph:
    ...
    if story_arc_type == "exile_rise":
        ...
        graph.add_causal_link(
            "pleasure_5", "pleasure_6", strength=0.8, amplification=1.5
        )  # 最終決戦→敵崩壊
