...
WORKFLOW_API_MAP: Dict[WorkflowType, Tuple[str, List[str]]] = {
    ...
    WorkflowType.PLOT_EXPANSION: ("expand_plots", ["book_id", "gen_from", "gen_to"]),
    WorkflowType.PLOT_REBUILD: ("rebuild_plots", ["params"]),
    WorkflowType.CRITIQUE_OPTIMIZE: ("critique_optimize", ["book_id"]),
    WorkflowType.PLAN_GENERATION: ("plan_generation", ["params"]),
    WorkflowType.RETRY_FAILED: ("retry_failed_episodes", ["book_id", "passion", "word_count"]),
    WorkflowType.IMPORT_CHAPTER: (
        "import_chapter",
        ["book_id", "ep_num", "import_text", "do_refine"],
    ),
    WorkflowType.MARKETING_GENERATION: ("generate_marketing", ["book_id", "latest_ep"]),
