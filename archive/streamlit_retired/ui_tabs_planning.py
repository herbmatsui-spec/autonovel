...
def render_easy_mode(state: Any, engine: UltimateHegemonyEngine) -> None:
    ...
    if not UIStateStore().ui_state.form_data.get("easy_mode_loaded", False):
        with st.container():
            st.markdown(
                '<div class="skeleton-header skeleton-fade-in"></div>', unsafe_allow_html=True
            )
            st.markdown(
                '<div class="skeleton-text skeleton-fade-in"></div>', unsafe_allow_html=True
            )
            st.markdown(
                '<div class="skeleton-card skeleton-fade-in"></div>', unsafe_allow_html=True
            )
    ...
    archetype_options = [
        k for k, v in EASY_GENRES.items() if not v.get("nsfw_only", False) or nsfw_enabled
    ]
    ...
    with st.expander(f"{ICON_SETTINGS} 細かい設定"):
        ...
        initial_limit = st.number_input(
            "最初に生成するプロット数", 5, 50, value=min(target_eps, 25)
        )
        word_count = st.number_input(
            "1話あたりの目標文字数", 1000, 5000, value=default_word_count, step=100
        )

        # 提案2: オプションの「コアアイデア」テキスト入力
        core_idea = st.text_area(
            "あなたのアイデア（例: 主人公は実は猫だった）",
            placeholder="ここに物語の核となるユニークなアイデアを書いてください（オプション）",
            height=100,
        )

        # 提案6: ユーザーフレンドリーな「物語の雰囲気」スライダー
        tone_vibe_slider = st.slider(
            "物語の雰囲気",
            -5,
            5,
            0,
            help="-5: 極めてシリアス/重厚, 0: 標準, +5: 極めてコミカル/軽快",
        )
    ...
    estimated_cost_usd = (estimated_tokens * COST_INPUT_FLASH) + (
        estimated_tokens * COST_OUTPUT_FLASH
    )
    ...
    with st.container(border=True):
        ...
        col_c.metric(
            ...
            help="Gemini 1.5 Flash の料金体系に基づいた概算です。",
        )
        col_t.metric(
            "推定時間", f"{estimated_time_min:.0f} 分", help="APIの生成速度に基づく概算です。"
        )
...
def _wizard_step_indicator(current: int, total: int = 4) -> None:
    ...
    if w["step"] == 1:
        ...
        with col1:
            w["genre"] = st.selectbox(
                "📚 ジャンル", genre_options, help="物語のベースとなるジャンルを選択してください。"
            )
        ...
        with col2:
            ...
            for idx, (name, preset) in enumerate(archetypes_data.items()):
                with cols[idx % 3]:
                    ...
                    if st.button(
                        f"✨ {name} を選択", key=f"select_arch_{name}", use_container_width=True
                    ):
                        w["archetype"] = name
                        w.update(
                            {
                                ...
                                if getattr(preset, k) is not None
                            }
                        )
        ...
        with col1:
            w["target_eps"] = st.number_input(
                "目標話数", 10, 200, w["target_eps"], help="完結までに見込むおおよその話数です。"
            )
            ...
            w["platform"] = st.selectbox(
                "🎯 ターゲットプラットフォーム",
                platforms,
                index=platforms.index(w.get("platform", "カクヨム/なろう"))
                if w.get("platform", "カクヨム/なろう") in platforms
                else 0,
            )
        with col2:
            # --- パラメータプリセットカード (提案6) ---
                    ...
                    for idx, tab in enumerate(tabs):
                        ...
                        with tab:
                            ...
                            ref_items = [
                                ...
                                (
                                    "🦸 主人公像案",
                                    c.refined_mc_suggestion,
                                    "mc_concept",
                                    "主人公像を反映",
                                ),
                                (
                                    "😈 敵対者像案",
                                    c.refined_villain_suggestion,
                                    "villain_concept",
                                    "敵対者像を反映",
                                ),
                            ]
                            ...
                            for label, value, key, btn_text in ref_items:
                                with st.container(border=True):
                                    ...
                                    if col_btn.button(
                                        btn_text, key=f"ref_{key}_{idx}", use_container_width=True
                                    ):
                                        w[key] = value
                                        UIStateStore.get().wizard.data = w
...
def render_planning_tab(state: Dict[str, Any], engine: UltimateHegemonyEngine) -> None:
    ...
    mode = st.radio(
        "入力モード", [f"{ICON_PLANNING} ウィザード", f"{ICON_SETTINGS} 詳細設定"], horizontal=True
    )
    ...
    if "ウィザード" in mode:
        ...
        render_section_header(
            "企画詳細設定", "高度なパラメータを直接指定して企画を生成します", ICON_SETTINGS
        )
        ...
        with st.form("detail_plan_form"):
            ...
            with col1:
                ...
                genre_options = [plugin.genre] if plugin else ["ファンタジー"]
                genre = st.selectbox(
                    "ジャンル", genre_options, help="作品の基本ジャンルを選択してください"
                )
                keywords = st.text_area(
                    "キーワード",
                    "追放, チート, ざまぁ",
                    placeholder="カンマ区切りで入力してください",
                    help="物語の核となるキーワードを記述します",
                )
                target_eps = st.number_input(
                    "目標話数", 10, 200, 50, help="完結までに想定している総話数です"
                )
            with col2:
                archetype = st.selectbox(
                    "アーキタイプ",
                    list(STORY_ARCHETYPES.keys()),
                    help="物語の構造的な型を選択してください",
                )
                ...
                initial_limit = st.number_input(
                    "初期生成話数", 1, 50, 25, help="最初に一括生成するプロットの数です"
                )
            ...
            with st.expander("🛠️ 高度なエンジンスペック設定"):
                ...
                with e_col1:
                    cheat_scale = st.slider(
                        "チート強度", 1, 10, 4, help="主人公の能力の突出度合いを調整します"
                    )
                with e_col2:
                    system_assist = st.slider(
                        "システム支援率", 0, 100, 70, help="AIによる構成支援の介入度合いです"
                    )
                with e_col3:
                    cost_severity = st.slider(
                        "コスト厳格度", 1, 5, 2, help="トークン消費の最適化レベルを調整します"
                    )
                ...
                with e_col1:
                    ...
                    catharsis_label_options = {
                        v["label"]: k for k, v in CATHARSIS_DENSITY_PRESETS.items()
                    }
                    ...
                    catharsis_density_preset_index = catharsis_label_options[
                        catharsis_label_selected
                    ]
                with e_col2:
                    min_immersion = st.slider(
                        "最低没入スコア", 0.0, 100.0, 0.0, 1.0, help="品質ゲート用の最小没入スコア"
                    )
