...
def render_monitor_tab(state, engine: EngineService, book_id: int):
    ...
    if target_eps > 0:
        ...
        st.caption(
            f"目標 {target_eps} 話中 {stats['total_chapters']} 話完了 ({progress_pct * 100:.1f}%)"
        )
    else:
    ...
    with st.expander(f"{ICON_SETTINGS} 物語制御 (デバッグ用)"):
        st.caption(
            "物語の状態や遷移パスを強制的に変更します。次回のパイプライン実行時に反映されます。"
        )
        ...
        with col_path:
            target_path = st.selectbox(
                "優先遷移パス", ["standard", "twist"], key="monitor_target_path"
            )
