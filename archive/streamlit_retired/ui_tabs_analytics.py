...
def render_strategy_tab(engine: UltimateHegemonyEngine, book_id: int) -> None:
            ...
            if bible and bible.settings:
                try:
                    settings = (
                        ...
                        else json.loads(bible.settings)
                    )
                except Exception:
                    settings = {}
                st.write("**現在のアーク構成:**")
                for arc in settings.get("arcs", []):
                    st.caption(
                        f"第{arc.get('start_ep')}~{arc.get('end_ep')}話: {arc.get('title')} - {arc.get('summary')}"
                    )
    ...
    with t_logic:
        ...
        if not chapters:
            ...
            try:
                ws = (
                    last_ch.world_state
                    if isinstance(last_ch.world_state, dict)
                    else json.loads(last_ch.world_state or "{}")
                )
            except:
                ws = {}
            ...
            t_mem, t_thread = st.tabs(
                [f"{ICON_BOOK} 長期記憶管理", f"{ICON_FLAG} 伏線元帳エディタ"]
            )

            with t_mem:
                st.markdown("#### 累積あらすじ")
                summary = st.text_area(
                    "AIが維持している物語の要約", ws.get("cumulative_summary", ""), height=200
                )
                if st.button("記憶を上書き保存"):
                    ws["cumulative_summary"] = summary
                    engine.create_chapter(
                        book_id=book_id,
                        ep_num=last_ch.ep_num,
                        title=last_ch.title,
                        content=last_ch.content,
                        summary=last_ch.summary,
                        killer_phrase=last_ch.killer_phrase,
                        ai_insight=str(last_ch.ai_insight or ""),
                        world_state=ws,
                        trinity_review_log=last_ch.trinity_review_log,
                        created_at=last_ch.created_at,
                    )
            ...
            with t_thread:
                ...
                if threads:
                    ...
                    if HAS_PLOTLY:
                            ...
                            if payoff and payoff > setup:
                                fig_threads.add_trace(
                                    go.Scatter(
                                        ...
                                        marker=dict(
                                            size=[8, 10], symbol=["circle", "star"], color=color
                                        ),
                                        name=status_ja,
                        ...
                        fig_threads.update_layout(
                            ...
                            yaxis=dict(
                                title="", automargin=True, gridcolor="rgba(255,255,255,0.05)"
                            ),
                        ...
                        st.info(
                            "📊 タイムラインをより見やすく描画するには `pip install plotly` を環境に導入してください。"
                        )
                        # 簡易表示
                            ...
                            [
                                {
                                    ...
                                    "回収予定話数": f"第{t.get('target_resolve_episode')}話"
                                    if t.get("target_resolve_episode")
                                    else "未定",
                                    "ステータス": t.get("status"),
                                }
                                for t in threads
                ...
                with st.expander("➕ 新しい伏線を手動で追加する"):
                    with st.form("add_thread_form"):
                        ...
                        t_payoff = st.number_input(
                            "回収予定話数 (Payoff / 任意 - 0は未設定)", 0, 200, 0
                        )
                ...
                for i, t in enumerate(threads):
                    with st.container(border=True):
                        ...
                        new_status = c3.selectbox(
                            "状態",
                            ["Active", "Closed", "Resolving", "Dormant"],
            ...
            audit_log = (
                (last_ch.trinity_review_log or {}).get("audit_log", {})
                if isinstance(last_ch.trinity_review_log, dict)
                else {}
            )
            if audit_log.get("conflict_points"):
                ...
                st.success(
                    f"{ICON_STAR} 物語の整合性は正常です。AIは過去の全事実を正確に把握しています。"
                )
            ...
            if audit_log.get("missing_items"):
                ...
                st.warning(
                    f"{ICON_WARNING} **未回収の予定伏線:** {', '.join(audit_log['missing_items'])}"
                )
    ...
    with t_stress:
        ...
        with col_s1:
            ...
            st.progress(
                min(1.0, current_stress / 65),
                text=f"ヘイト飽和率: {int(min(1.0, current_stress / 65) * 100)}%",
            )
        ...
        if current_stress >= 65:
            st.warning(
                "🔥 **ストレスが限界値を超えています**。次話は自動的に『大ざまぁ（強制カタルシス）』として執筆され、読者の欲求を爆発させます。"
            )
        else:
            st.info(
                "🟢 ストレスは健全な範囲内です。敵の理不尽さを維持しつつ、Payoff（解決）に向けて溜めを作ってください。"
            )
...
def _render_optimization_cockpit(engine, book_id, book):
    ...
    if st.button(
        f"{ICON_ROCKET} 10 Iterations 自己最適化分析を実行",
        type="primary",
    ...
    if display_meta:
        ...
        with col_r:
            with st.container(border=True):
                ...
                patch_data = (
                    meta.get("config_patch") or meta.get("config.py への具体的修正案") or ""
                )
            ...
            with st.container(border=True):
                ...
                prompt_patch = (
                    meta.get("prompt_patch") or meta.get("engine_prompts.py への具体的修正案") or ""
                )
        ...
        with col1:
            ...
            if not pending_patches:
                ...
                for patch in pending_patches:
                    with st.container(border=True):
                        st.markdown(
                            f"**タイプ:** `{patch['patch_type'].upper()}` | **作成日:** `{patch['created_at']}`"
                        )
                        ...
                        st.code(
                            patch["patch_content"],
                            language="python" if patch["patch_type"] == "config" else "markdown",
                        )
                        ...
                        # Approve Action
                        if col_btn1.button(
                            f"👍 承認して適用 (ID: {patch['id']})", key=f"approve_{patch['id']}"
                        ):
                            from streamlit_app import api_client
                        ...
                        # Reject Action
                        if col_btn2.button(
                            f"👎 却下 (ID: {patch['id']})", key=f"reject_{patch['id']}"
                        ):
                            from streamlit_app import api_client
            ...
            if not prompt_versions:
                ...
                for ver in prompt_versions:
                    with st.container(border=True):
                        ...
                        st.markdown(
                            f"**評価スコア:** 事前: `{score_before or 'N/A'}` | 事後: `{score_after or 'N/A'}`"
                        )
                        ...
                        st.text_area(
                            ...
                            ver["content"],
                            height=80,
                            key=f"ver_content_{ver['id']}",
                            disabled=True,
                        )

                        if not ver["is_active"] and not ver["rollback_reason"]:
                            if st.button(
                                f"⏪ このバージョンにロールバック (ID: {ver['id']})",
                                key=f"rollback_{ver['id']}",
                            ):
                                from streamlit_app import api_client
...
def _display_radar_chart(meta):
    ...
    fig = go.Figure(
        data=go.Scatterpolar(r=values, theta=categories, fill="toself", line_color="#e94560")
    )
    fig.update_layout(
...
def render_prompt_metrics_dashboard():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with col3:
                st.metric(
                    "エラー率",
                    f"{(sum(m.get('error_count', 0) for m in metrics) / max(sum(m.get('hits', 0) for m in metrics), 1) * 100):.1f}%",
                )
            with col4:
            ...
            st.subheader("詳細データ")
            display_df = df[
                ["template_name", "hits", "avg_time_ms", "error_count", "last_accessed"]
            ].copy()
            if "last_accessed" in display_df.columns:
                display_df["last_accessed"] = pd.to_datetime(
                    display_df["last_accessed"]
            ...
            with st.expander("データ管理"):
                ...
                if st.button("古いデータを削除"):
                    delete_response = requests.delete(
                        f"http://localhost:8200/api/prompt-metrics?days={days}"
                    )
    ...
    with t_erotic:
        ...
        if not session.config.get("enable_nsfw", False):
            st.warning(
                "🔞 官能度監査を利用するには、サイドバーで「NSFWモード」を有効にしてください。"
            )
        else:
