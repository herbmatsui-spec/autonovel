...
class HegemonyKernel(KernelBase):
    ...
    async def generate_hegemony_event(self, context):
        ...
        if not patterns_list:
            return {
                ...
                "dynamic": dynamic,
                "instruction": "Generic power event",
            }
        ...
        for p in patterns_list:
            if "high_tension" in p.get("triggers", []) and tension > 70:
                ...
                break
            if (
                "resonance_increase" in p.get("triggers", [])
                and getattr(analytics, "resonance", 0) > 70
            ):
                selected_pattern = p
                break
        ...
        interpretation = self._calculate_pov_interpretation(
            pov_char, dynamic, event_type, selected_pattern
        )

        # 視点による解釈の齟齬（誤読層）を追加
        misinterpretation = self._calculate_misinterpretation(
            pov_char, dynamic, event_type, interpretation
        )
        ...
        # 監査ループの実行
        final_scene = await self._run_hegemony_audit(
            context, base_instruction, event_type, dynamic, interpretation, misinterpretation
        )
        ...
        return {
            ...
            "enigma_link": {
                ...
                "secret_exposed": dynamic.keystone_vulnerability
                if event_type in ["SHAKE", "COLLAPSE"]
                else None,
                "witness": pov_char,
            },
            "scene": final_scene,
    ...
    def analyze_power_gap(self, char_a: str, char_b: str, context) -> PowerDynamic:
        ...
        fragility = (
            char_a_data.get("internal_fragility", 30.0)
            if dom == char_a
            else char_b_data.get("internal_fragility", 30.0)
        )

        # 急所の特定
        vulnerability = (
            char_a_data.get("keystone_vulnerability", "Unknown vulnerability")
            if dom == char_a
            else char_b_data.get("keystone_vulnerability", "Unknown vulnerability")
        )
    ...
    def _calculate_misinterpretation(
        self, pov_char: Optional[str], dynamic: PowerDynamic, event_type: str, interpretation: Dict
    ) -> Dict:
        """
    ...
    def _calculate_pov_interpretation(
        self, pov_char: Optional[str], dynamic: PowerDynamic, event_type: str, pattern: Dict
    ) -> Dict:
        """
        ...
        if pov_char == dynamic.dominant_char:
            ...
            if event_type == "COLLAPSE":
                return {
                    "mode": "panic",
                    "reaction": "Shock and desperation as the structure crumbles",
                }
            if event_type == "SHAKE":
                return {
                    "mode": "anxiety",
                    "reaction": "Fear of exposure and urgent need to regain control",
                }
            ...
            if event_type == "COLLAPSE":
                return {
                    "mode": "liberation",
                    "reaction": "Euphoria and newfound power as the oppressor falls",
                }
            if event_type == "SHAKE":
                return {
                    "mode": "discovery",
                    "reaction": "Intense curiosity and calculated hope upon finding a weakness",
                }
            return {
                "mode": "resignation",
                "reaction": "Internalization of helplessness and submission",
            }
            ...
            if event_type == "COLLAPSE":
                return {
                    "mode": "astonishment",
                    "reaction": "Bafflement at the sudden reversal of status",
                }
            if event_type == "SHAKE":
                return {
                    "mode": "suspicion",
                    "reaction": "Sensing an underlying tension that doesn't match the surface hierarchy",
                }
    ...
    def analyze_commercial_hegemony_structure(
        characters: Dict[str, Dict], story_progress: float = 0.0
    ) -> List[PowerDynamic]:
        """商用役割を考慮した権力構造を分析
        ...
        for name, char_data in characters.items():
            ...
            structural_gap = abs(
                adjusted_power - characters.get(protagonist, {}).get("power_level", 50)
            )
