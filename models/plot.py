...
class CliffhangerDef(BaseModel):
    ...
    tension_type: str = Field(
        default="Standard", description="引きの種類 (Cliffhanger, Question, Revelation, Emotional)"
    )
...
def ensure_cliffhanger_obj(v: Any) -> Any:
    if isinstance(v, dict):
        return CliffhangerDef(**v)
    if isinstance(v, str):
        return CliffhangerDef(hook_text=v)
    return v
...
class MasterSceneBlock(BaseModel):
    ...
    def ensure_beats_list(cls, v: Any) -> Any:
        if isinstance(v, list):
            return v
        if isinstance(v, dict) and "beats" in v:
            return v["beats"]
        return []
...
class PlotCoreInfo(BaseModel):
    ...
    title: str = Field(
        default="", description="話題", validation_alias=AliasChoices("title", "episode_title")
    )
...
class PlotAnalytics(BaseEngine):
    ...
    love_meter: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0, description="愛/信頼度の変動"
    )
...
class PlotEpisodeBase(CoreEngineMixin):
    ...
    def unwrap_plot_metadata(cls, data: Any) -> Any:
        ...
        remaining_keys = [
            ...
            if k not in generic_fields and k not in sub_models_names and k != "extra_engines"
        ]
...
class PlotEpisode(PlotEpisodeBase, EnigmaMixin, ComfortMixin):
    ...
    current_chain_phase: Annotated[str, BeforeValidator(normalize_chain_phase)] = Field(
        default="Friction",
    ...
    target_tension: Annotated[float, BeforeValidator(extract_float)] = Field(
        default=0.0, description="動的目標テンション値"
    )
