"""
src/streamlit_app/ui_tabs_writing.py — 小説生成タブコンポーネント（最終強化版）
"""
...
if "api_retry_state" not in st.session_state:
    st.session_state.api_retry_state = {"attempts": 0, "max_attempts": 3, "backoff": 1}
if "poll_interval" not in st.session_state:
    st.session_state.poll_interval = 1
...
def retry_api(func):
    ...
    def wrapper(*args, **kwargs):
        state = st.session_state.api_retry_state
        state["attempts"] += 1
...
def render_novel_production_tab():
        ...
        if "commercial_task_id" not in st.session_state:
            if st.button("商用化実行"):
                ...
                try:
                    ...
                    st.session_state.commercial_task_id = task_id
                    ...
                    # ---------------  進捗ポーリング ----------
                    # -------------------------------------------------
                    if st.session_state.commercial_task_id:
                        ...
                        while elapsed < max_wait_seconds:
                            ...
                            try:
                                status_resp = httpx.get(
                                    f"http://localhost:8000/api/tasks/{st.session_state.commercial_task_id}/status",
                                    timeout=5.0
                                )
    ...
    # 停止ボタン（生成中のみ表示）
    # ----------------------------------------------------------------------
    if "gen_task_id" in st.session_state:
        if st.button("生成停止"):
            try:
                stop_resp = httpx.post(
                    f"http://localhost:8000/api/tasks/{st.session_state.gen_task_id}/stop",
                    timeout=10.0
                )
                ...
                del st.session_state["gen_task_id"]
