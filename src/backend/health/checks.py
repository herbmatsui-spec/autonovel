...
async def check_database(db_manager) -> HealthCheckResult:
    ...
    try:
        ...
        return HealthCheckResult(
            ...
            latency_ms=latency,
            details=f"pool={checked_in}/{pool_size}"
        )
...
async def check_redis(redis_url: Optional[str]) -> HealthCheckResult:
    ...
    if not redis_url:
        return HealthCheckResult(status=HealthStatus.NOT_CONFIGURED, error="REDIS_URL not configured")
    start = time.perf_counter()
    try:
...
async def check_chromadb() -> HealthCheckResult:
    ...
    try:
        ...
        if not client:
            return HealthCheckResult(status=HealthStatus.NOT_CONFIGURED, error="ChromaDB not initialized")
        client.heartbeat()
