...
class ContextManager:
    ...
    def _parse_character_registry(char) -> dict:
        ...
        try:
            ...
            logger.warning(f"Failed to parse registry_data for character '{char_name}': {e}")
            return {}
        except Exception as e:
            logger.error(f"Unexpected error reading character registry for '{char_name}': {e}")
            return {}
    ...
    async def build_past_context(
            ...
            try:
                ...
                for chap in all_past[:10]:
                    ws = (
                        ...
                        if isinstance(chap.world_state, str)
                        else chap.world_state
                    )
                    ...
                    if cumulative_summary and threads:
                        break
            except (json.JSONDecodeError, AttributeError) as e:
            ...
            for t in threads:
                if isinstance(t, dict):
                    ...
                    if active_char_names:
                        ...
                        if not has_active_char and any(
                            k in desc_str for k in char_states.keys() if k not in active_char_names
                        ):
                            continue
        ...
        for c in reversed(recent):
            ctx += f"- 第{c.ep_num}話: {c.summary} (確定事実: {c.ai_insight if c.ai_insight else 'なし'})\n"
        return ctx.strip()
    ...
    async def get_optimal_context(
        ...
        if all_past:
            ws = (
                all_past[0].world_state
                if isinstance(all_past[0].world_state, dict)
                else json.loads(all_past[0].world_state or "{}")
            )
            char_states = ws.get("character_states", {})
        ...
        # 基本コンテキスト
        base_past_ctx = await self.build_past_context(
            book_id, ep_num, active_char_names=active_char_names
        )
    ...
    async def get_optimal_context_split(
        ...
        if all_past:
            try:
                ws = (
                    json.loads(all_past[0].world_state)
                    if isinstance(all_past[0].world_state, str)
                    else all_past[0].world_state
                )
                char_states = ws.get("character_states", {})
        ...
        prev_ctx = await self.build_past_context(
            book_id, ep_num, active_char_names=active_char_names
        )
