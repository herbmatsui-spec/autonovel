...
from src.models import ChapterDbModel, NarrativeWavePattern, PlotDbModel
...
class PacingGraph:
    ...
    def get_instruction(ep_num: int, total_eps: int = 50, is_light: bool = False) -> dict[str, Any]:
        ...
        if ep_num == 1:
            if is_light:
                return {
                    "instruction": "【第1話】主人公の日常的な『ついてなさ』をコミカルに描写。チート能力による快進撃の予感を描け。",
                    "density": "情報密度: 低",
                    "temp": 0.85,
                }
            return {
                "instruction": "【第1話】主人公の理不尽な不遇・絶望を五感で執拗に描写。最後に能力の片鱗が現れるが、無双で終わらせるな。",
                "density": "情報密度: 低",
                "temp": 0.8,
            }
...
class NarrativeController:
    ...
    def get_integrity_threshold(
        self, genre: str, prev_integrity: int = 100, engine_key: str = ""
    ) -> float:
        base = 0.6
        genre_lower = genre.lower()
...
class WavePatternAnalyzer:
    ...
    def _calculate_wave_score(
        ...
        ratio_score = (
            min(100.0, (catharsis_count / expected_catharsis) * 100)
            if expected_catharsis > 0
            else 0.0
        )
