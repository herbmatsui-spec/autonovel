...
async def compare(book_id: int, req: CompareRequest) -> Dict[str, Any]:
    ...
    if n == 0:
        ...
        raise HTTPException(status_code=404, detail="比較対象のプロンプトバージョンが見つかりません")
    versions = versions[:n]
    texts = req.texts[:n]
...
async def activate_version(book_id: int, version_id: int) -> Dict[str, Any]:
    ...
    async with UnitOfWork(Container.db()) as uow:
        ...
        row = (await uow.session.execute(select(PromptVersion).where(PromptVersion.id == version_id))).scalar_one_or_none()
        if row is None:
            from src.core.exceptions import NotFoundError

            raise NotFoundError("PromptVersion not found", resource_type="PromptVersion", resource_id=str(version_id))
