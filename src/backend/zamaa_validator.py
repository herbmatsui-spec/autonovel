    ...
    async def validate_plot_arc(
        ...
        if engine_key != "zamaa":
            return ZamaaAuditResult(
                ...
                score=100,
                issues=[],
                suggestion="Not a Zamaa project, skipping specialized validation.",
            )
        ...
        if not scenes:
            return ZamaaAuditResult(
                is_valid=False,
                score=0,
                issues=["No scenes found in plot data."],
                suggestion="Please generate scenes first.",
            )
        ...
        for scene in early_scenes:
            ...
            if any(
                word in action
                for word in ["humiliation", "unfair", "betrayal", "abuse", "軽視", "追放", "不当"]
            ):
        ...
        if not stress_detected:
            issues.append(
                "Stress accumulation phase is weak or missing. Readers may not feel the need for retribution."
            )
        ...
        for scene in scenes:
            ...
            if any(
                word in action
                for word in ["trigger", "awakening", "reveal", "覚醒", "露見", "転換"]
            ):
                has_trigger = True
                break
        ...
        if not has_trigger:
            issues.append(
                "Clear trigger for retribution is missing. The transition from stress to catharsis is too abrupt or vague."
            )
        ...
        suggestion = (
            "Ensure a clear 4-act structure: Humiliation -> Trigger -> Retribution -> Dominance."
        )
        if not stress_detected:
            suggestion = (
                "Increase the 'unfairness' in the first half to amplify the eventual catharsis."
            )
        elif not catharsis_detected:
            suggestion = "Make the retribution more explicit and satisfying."

        return ZamaaAuditResult(
            is_valid=is_valid, score=score, issues=issues, suggestion=suggestion
        )
