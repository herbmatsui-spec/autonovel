...
class StyleRagManager:
    ...
    async def find_best_samples(self, scene_description: str, phase: str = "Hate", trope_hint: Optional[str] = None, top_k: int = 2) -> List[str]:
        ...
        if trope_hint:
            # 頻繁に参照される全サンプルリストをキャッシュする仕組み
            # ここでは簡易的に repo 側でキャッシュされていることを期待するが、
            # 呼び出し側でも一度取得したものを保持することでDBアクセスを減らす
            candidates = await self.repo.get_all_style_fragments(tag=trope_hint)
            if not candidates:
                candidates = await self.repo.get_all_style_fragments()
        else:
            candidates = await self.repo.get_all_style_fragments()
