...
class BaseWorkflow(ABC):
    ...
    def __init__(
        ...
        self.writing: WritingPort = writing if writing is not None else (engine.writer if engine else None)
        # PlanningService: 注入されなければ engine.planner で代用
        self.planner: Optional[PlanningService] = planner if planner is not None else (engine.planner if engine else None)
        # WritingService: 注入されなければ engine.writer で代用（プロトコル互換）
        self.writing_service: Optional[WritingService] = writing_service if writing_service is not None else (engine.writer if engine else None)
        # DataRepository: 注入されなければ engine.repo で代用
        self.repo: Optional[DataRepository] = repo if repo is not None else (engine.repo if engine else None)
        # その他のサービスポート
        self.critique: Optional[CritiquePort] = critique if critique is not None else (engine.critique if engine else None)
        self.narrative: Optional[Any] = narrative if narrative is not None else (engine.narrative if engine else None)
        self.marketing: Optional[Any] = marketing if marketing is not None else (engine.marketing if engine else None)
        self.bible_agent: Optional[BiblePort] = bible_agent if bible_agent is not None else (engine.bible_agent if engine else None)
        self.plot_agent: Optional[Any] = plot_agent if plot_agent is not None else (engine.plot_agent if engine else None)
        self.formatter: Optional[Any] = formatter if formatter is not None else (engine.formatter if engine else None)
        # Prefetch 用
