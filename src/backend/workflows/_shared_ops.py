...
async def enqueue_shadow_audit(
    book_id: int, write_from: int, write_to: int, reporter: StatusReporter
) -> None:
...
async def trigger_prefetch(
    engine, book_id: int, last_episode: int, reporter: StatusReporter
) -> None:
