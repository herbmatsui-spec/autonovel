...
class CritiqueAgent:
    ...
    async def analyze_work_quality(self, book_id: int) -> str:
        ...
        prompt = await self.pm.build_critique_quality_prompt(
            book.title, json.dumps(summary_data, ensure_ascii=False, indent=2), book_id=book_id
        )
        res = await self.generate_json(MODEL_PLANNING, prompt, temp=0.3)
    ...
    async def run_iterative_gap_analysis(
        ...
        if not chapters:
            return GenerateResult(
                success=False, error_message="分析対象のチャプターが存在しません。"
            )
        ...
        batch_analysis_prompt = await self.pm.build_iterative_gap_analysis_prompt(
        ...
        while attempt < max_attempts:
            ...
            res = await self.generate_json(MODEL_PLANNING, current_prompt, temp=0.3)
            ...
            if not errors:
                ...
                error_feedback = "\n".join(errors)
                logger.warning(
                    f"❌ Patch validation failed on attempt {attempt + 1}: {error_feedback}"
                )
                attempt += 1

        # 全て失敗した場合
        logger.error(
            "❌ Max critique self-repair attempts reached. Saving optimization report anyway without pending patches."
        )
        await self.repo.save_optimization_report(book_id, res.metadata)
        return GenerateResult(
            success=False,
            error_message=f"パッチの自己修復バリデーションに失敗しました: {error_feedback}",
        )
    ...
    async def run_dry_run(self, book_id: int, ep_num: int, improved_prompt: str) -> str:
        ...
        prompt = await self.pm.build_dry_run_prompt(
