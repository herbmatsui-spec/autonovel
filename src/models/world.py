        ...
        if isinstance(data, dict):
            ...
            if "causality_map" in data:
                ...
                if isinstance(val, str):
                    if "\n" in val:
                        data["causality_map"] = [
                            line.strip("- *").strip() for line in val.split("\n") if line.strip()
                        ]
                    elif "→" in val:
                        data["causality_map"] = [
                            part.strip() for part in val.split("→") if part.strip()
                        ]
                    elif "->" in val:
                        data["causality_map"] = [
                            part.strip() for part in val.split("->") if part.strip()
...
class ForeshadowingAudit(BaseModel):
    ...
    def unwrap_and_validate_items(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            if "is_recovered" not in data and not any(
                a in data for a in ["recovered", "isRecovered"]
            ):
