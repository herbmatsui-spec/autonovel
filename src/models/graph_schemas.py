...
class Entity(BaseModel):
    ...
    name: str = Field(
        ..., description="エンティティの名前 (例: アルス, 聖剣エクスカリバー, 王都ルミナス)"
    )
    type: Literal["Character", "Location", "Item", "Event", "Faction", "Concept"] = Field(
        ..., description="エンティティの種別"
    )
    description: str = Field(
        "", description="この章における簡潔な説明や状態 (例: 負傷中, 宝物庫に保管)"
    )
    properties: dict[str, Any] = Field(
        default_factory=dict, description="追加の属性情報 (例: is_alive, faction)"
    )
...
class Relationship(BaseModel):
    ...
    type: str = Field(
        ..., description="関係種別 (例: KNOWS, HATES, ALLY_OF, LOCATED_IN, POSSESSES, ATTACKED)"
    )
...
class GraphExtractionResult(BaseModel):
    ...
    relationships: list[Relationship] = Field(
        default_factory=list, description="抽出された関係性一覧"
    )
