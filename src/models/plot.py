...
class SceneBeat(BaseModel):
    ...
    emotion_phase: str = Field(
        default="neutral", description="感情フェーズ: buildup/explosion/aftermath"
    )
...
def normalize_beat_type(v: Any) -> str:
    ...
    exact_mapping = {
        ...
        "Setup": "導入",
        "In Medias Res": "導入",
        "Inciting Incident": "導入",
        "Inciting Event": "導入",
        "World Building": "導入",
        "World Building / Setup": "導入",
        "Development": "展開",
        "Rising": "展開",
        "Rising Action": "展開",
        "Escalation": "展開",
        ...
        "Background": "状況",
        "Truth": "状況",
        "Revelation of Truth": "状況",
        "Environmental Horror": "状況",
        "Atmosphere": "状況",
        "Situation": "状況",
        "Internal": "内面葛藤",
        "Thought": "内面葛藤",
        "Internal Conflict": "内面葛藤",
        "Reflection": "内面葛藤",
        "Mental Reconstruction": "内面葛藤",
        "Physical and Mental Decay": "内面葛藤",
        "Conflict": "内面葛藤",
        "Tension": "内面葛藤",
        "Dilemma": "内面葛藤",
        "Action": "具体的行動",
        "Battle": "具体的行動",
        "Retribution": "具体的行動",
        "Vow of Revenge": "具体的行動",
        "Interaction": "具体的行動",
        "Confrontation": "具体的行動",
        "Decision": "具体的行動",
        "Physical Action": "具体的行動",
        "Aftermath": "余韻",
        "Cliffhanger": "余韻",
        "Denouement": "余韻",
        "Quiet Foreshadowing": "余韻",
        "Calm": "余韻",
        "Echo": "余韻",
    }
...
class MasterSceneBlock(BaseModel):
    scene_number: int = Field(
        ...,
        description="シーン番号",
        validation_alias=AliasChoices("scene_number", "scene_id", "no", "id"),
    )
    action: str = Field(default="", description="このシーンで起きる主要な出来事")
    dialogue_point: str = Field(
        default="", description="このシーンで必ず交わされるべき重要な会話の要点"
    )
    ...
    bridge_instruction: str = Field(
        default="",
        description="前シーンからの時間・場所・情緒的な繋がりを自然にするための導入指示。",
    )
    impact_score: int = Field(
        default=50,
        ge=0,
        le=100,
        description="シーンの物語上の重要度。高いほど描写を詳細に（Show）、低いほど簡潔に（Tell）。",
    )
    psychological_layer: str = Field(
        default="",
        description="このシーンでのキャラクターの深層心理、内面的な葛藤、モノローグの指針。",
    )
...
class PlotBlueprintPhase1(BaseModel):
    ...
    next_hook: Annotated[CliffhangerDef, BeforeValidator(ensure_cliffhanger_obj)] = Field(
        default_factory=CliffhangerDef
    )
...
class PlotCoreInfo(BaseModel):
    ...
    one_line_summary: str = Field(
        default="", max_length=150, description="この話の核心を突く一行あらすじ"
    )
...
class PlotAnalytics(BaseEngine):
    tension: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=50, validation_alias=AliasChoices("tension", "intensity")
    )
    tension_delta: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0,
        description="緊張・摩擦の量",
        validation_alias=AliasChoices("tension_delta", "stress", "stress_delta"),
    )
    catharsis: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0,
        description="カタルシスの量",
        validation_alias=AliasChoices("catharsis", "catharsis_delta"),
    )
    is_catharsis: bool = Field(
        default=False, validation_alias=AliasChoices("is_catharsis", "catharsis_flag")
    )
    love_meter: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0,
        description="好感度の変化量",
        validation_alias=AliasChoices("love_meter", "love", "affinity"),
    )
    catharsis_type: str = Field(
        default="なし", validation_alias=AliasChoices("catharsis_type", "catharsis_style")
    )
    emotional_payoff: str = Field(
        default="", validation_alias=AliasChoices("emotional_payoff", "payoff")
    )
    resolution_style: str = Field(
        default="Cheat", validation_alias=AliasChoices("resolution_style", "style", "resolution")
    )
...
class PlotForeshadowing(BaseEngine):
    ...
    truth_convergence: float = Field(
        default=0.0,
        description="敵対者が真相に辿り着くまでの論理的収束度 (0.0 - 1.0)",
        validation_alias=AliasChoices("truth_convergence", "convergence"),
    )
    knowledge_friction: float = Field(
        default=0.0,
...
class EnigmaAnalytics(PlotForeshadowing):
    ...
    knowledge_delta: Annotated[float, BeforeValidator(extract_float)] = Field(
        default=0.0,
        description="周囲との知識/知能格差のスコア (0.0-1.0)",
        validation_alias=AliasChoices("knowledge_delta", "intel_gap"),
    )
    truth_convergence: Annotated[float, BeforeValidator(extract_float)] = Field(
        default=0.0,
        description="敵対者が真相に辿り着くまでの論理的収束度 (0.0 - 1.0)",
        validation_alias=AliasChoices("truth_convergence", "convergence"),
    )
    knowledge_friction: Annotated[float, BeforeValidator(extract_float)] = Field(
        default=0.0,
        description="主人公の策が環境（インフラ、常識）に阻まれる指数 (0.0 - 1.0)",
        validation_alias=AliasChoices("knowledge_friction", "friction"),
    )
...
class ComfortAnalytics(BaseEngine):
    qol_delta: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0, description="この話によるQOL向上値"
    )
...
class PlotEpisodeBase(FlatModelMixin, CoreEngineMixin, Generic[T]):
            ...
            if any(
                ...
                try:
                    if isinstance(ann, type) and (
                        issubclass(ann, BaseEngine) or issubclass(ann, PlotCoreInfo)
                    ):
                        names.append(k)
                except TypeError:
                    pass
    ...
    next_hook: Annotated[CliffhangerDef, BeforeValidator(ensure_cliffhanger_obj)] = Field(
        default_factory=CliffhangerDef,
        validation_alias=AliasChoices("next_hook", "cliffhanger", "hook"),
    )
    ...
    script_content: str = Field(
        ...
        validation_alias=AliasChoices(
            "script_content", "script", "manuscript", "content", "final_content"
        ),
    )
    current_chain_phase: Annotated[ChainPhase, BeforeValidator(normalize_chain_phase)] = Field(
        default="Friction",
        validation_alias=AliasChoices("current_chain_phase", "phase", "chain_phase"),
    )
    burned_cost_or_loot: Annotated[str, BeforeValidator(ensure_str)] = Field(
        default="なし", validation_alias=AliasChoices("burned_cost_or_loot", "cost", "loot")
    )
    ...
    lite_model_director_notes: str = Field(
        default="",
        validation_alias=AliasChoices(
            "lite_model_director_notes", "self_critique", "director_notes", "notes"
        ),
    )
    emotional_resonance_score: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0,
        description="感情共鳴スコア (1-100)",
        validation_alias=AliasChoices(
            "emotional_resonance_score", "resonance", "emotional_resonance"
        ),
    )
    thematic_depth_score: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0,
        description="テーマの深さスコア (1-100)",
        validation_alias=AliasChoices("thematic_depth_score", "depth", "thematic_depth"),
    )
    literary_beauty_score: Annotated[int, BeforeValidator(extract_int)] = Field(
        default=0,
        description="文章の美しさスコア (1-100)",
        validation_alias=AliasChoices("literary_beauty_score", "beauty", "literary_beauty"),
    )
    ...
    def unwrap_plot_metadata(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            for sub_name in sub_models_names:
                model_cls = (
                    static_routing_map.get(sub_name) or cls.model_fields[sub_name].annotation
                )
            ...
            generic_fields = {
                ...
                "literary_beauty_score",
            }

            remaining_keys = [
                k
                for k in data.keys()
                if k not in all_routed_keys
                and k not in generic_fields
                and k not in sub_models_names
            ]
            if remaining_keys:
                extra = {}
...
class RoadmapItem(FlatModelMixin, BaseModel):
    ...
    one_line_summary: str = Field(
        ...,
        description="不可逆に前進する1行あらすじ",
        validation_alias=AliasChoices("one_line_summary", "one_line", "summary", "outline"),
    )
    resolution_style: Literal["Cheat", "Logic", "Focus_Drama"] = Field(
        ..., validation_alias=AliasChoices("resolution_style", "style", "resolution")
    )
...
class ArcBlueprint(BaseModel):
    arc_num: int = Field(
        ..., validation_alias=AliasChoices("arc_num", "arc_number", "number", "no")
    )
                    ...
                    if alias in data and data[alias] is not None:
                        try:
                            if target in ["arc_num", "start_ep", "end_ep"]:
                                data[target] = int(
                                    extract_int(data[alias])
                                    if hasattr(extract_int, "__call__")
                                    else data[alias]
                                )
                            else:
