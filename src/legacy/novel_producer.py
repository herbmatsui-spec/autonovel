...
class NovelProducer:
    ...
    async def generate_episode(self, project_id: int, ep_num: int) -> EpisodeResult:
        ...
        result = await self.episode_writer.write(
            book_id=project_id, ep_num=ep_num, context=self._build_context(ep_num)
        )
    ...
    def _build_context(self, ep_num: int) -> dict[str, Any]:
        ...
        context: dict[str, Any] = {
            "ep_num": ep_num,
            "is_first": ep_num == 1,
            "is_last": ep_num == self._current_project.target_episodes if self._current_project else False,
            "target_word_count": self._current_project.target_word_count_per_episode if self._current_project else 3000,
        }
