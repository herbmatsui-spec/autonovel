...
async def _simulate_single_reaction_async(
    char: dict[str, Any],
    journal: JournalEntry,
    author_name: str,
    llm: Any = None,
) -> SocialComment:
    """単一キャラクターのリアクション/コメントを非同期生成"""
...
"""
    ...
    if not content:
        if any(w in c_role for w in ["ライバル", "敵", "対立"]):
            reaction_type = "irony"
            content = "口では綺麗事を並べても、現実は非情だ。貴様のその甘さがいつまで続くか見ものだな。"
            trust_delta = -5.0
            tension_delta = 10.0
        elif any(w in c_role for w in ["師匠", "導き手", "親友"]):
            reaction_type = "support"
            content = "迷うことは恥ではない。その苦悩を抱えたまま、一歩前に進むのだ。"
            trust_delta = 10.0
            tension_delta = -5.0
        elif any(w in c_role for w in ["裏切り", "黒幕", "密定"]):
            reaction_type = "suspicion"
            content = "ほう、そう考えていたのか。だがその焦りこそが、こちらの好機となる。"
            trust_delta = -15.0
            tension_delta = 15.0
        else:
            reaction_type = "empathy"
            content = "その気持ち、分からなくもないよ。誰だって重い荷物を背負えば足が竦むものさ。"
            trust_delta = 5.0
            tension_delta = -2.0
...
async def simulate_character_reactions_async(
    journal: JournalEntry,
    other_characters: list[dict[str, Any]],
    llm: Any = None,
    max_reactions: int = 2,
) -> list[SocialComment]:
    """他キャラクターからのリアクションを asyncio.gather で並行シミュレート (Step 18)"""
    if not other_characters:
        return []
    ...
    selected = candidates[:max_reactions]
    tasks = [
        _simulate_single_reaction_async(
            char=char,
            journal=journal,
    ...
    valid_comments: list[SocialComment] = []
    for r in results:
        if isinstance(r, SocialComment):
            valid_comments.append(r)
        else:
            logger.error("Error simulating reaction: %s", r)
