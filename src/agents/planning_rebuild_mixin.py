...
class PlanningAgentRebuildMixin:
    ...
    async def rebuild_hegemony_plot(
        ...
        settings = self._safe_get_dict(
            bible.settings if bible and hasattr(bible, "settings") else None
        )
        ...
        past_context = (
            "\n".join([f"- 第{p.ep_num}話: {p.summary}" for p in past_plots])
            if past_plots
            else "過去の文脈なし"
        )
        ...
        if not arc_result.success:
            if reporter:
                reporter.report(
                    f"アークの再設計に失敗しました: {arc_result.error_message}", "error"
                )
            return []
        ...
        for ep_num in range(start_ep, new_total + 1):
            if reporter:
                reporter.update_progress(
                    ep_num - start_ep, new_total - start_ep + 1, f"第{ep_num}話のプロットを再構築中"
                )
            ...
            for attempt in range(max_retries):
                ...
                if not plot_result.success:
                    if reporter:
                        reporter.report(
                            f"第{ep_num}話の生成に失敗しました: {plot_result.error_message}",
                            "warning",
                        )
                    continue
                ...
                if reporter:
                    reporter.report(
                        f"⚖️ 第{ep_num}話: 生成されたプロットの監査を実行中... (試行 {attempt + 1}/{max_retries})",
                        "info",
                    )

                # 監査1: 論理整合性 (LogicalAuditor)
                logical_audit_issues: LogicalAuditIssueList = (
                    await self.engine.auditor.audit_plot_as_issues(
                        book_id=book_id,
                        branch_id=branch_id,
                        ep_num=ep_num,
                ...
                if logical_audit_issues.is_consistent and is_causal_ok:
                    ...
                    if reporter:
                        reporter.report(
                            f"⚠️ 第{ep_num}話: 監査で問題を検知しました。自動修復を試みます...: {last_error_summary}",
                            "warning",
                        )
        ...
        if new_plots:
            try:
                async with self.engine.uow:
                    if reporter:
                        reporter.report(
                            f"💾 データベースを更新中... (第{start_ep}話以降の古いプロットをアーカイブし、新しいプロットを保存します)",
                            "info",
                        )
