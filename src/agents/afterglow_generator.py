...
class AfterglowGenerator:
    ...
    async def generate_monologue(
        ...
        fallback_by_mood = {
            "wary": f"（……信じられない。あの男、何を企んでいるの？ でも……あの瞳、嘘をついているようには見えなかった……）",
            "tsundere": (
                f"（……あんな真剣な目で見つめられたら、怒る気も失せるじゃない。"
                f"『お前が無事でよかった』だなんて……ばか。私の全部、見透かされてるみたいで悔しい……）"
            ),
            "affectionate": (
                f"（……温かいな。そばにいるだけで、胸の奥がじんわり熱くなる。"
                f"ずっと言えなかった『ありがとう』、次こそはちゃんと目を見て伝えよう……）"
            ),
            "deep_love": (
                f"（……もう、あなたなしの世界なんて考えられない。"
                f"誰にも渡さない。私の全部で、あなたを守り抜いてみせるから……）"
            ),
            "observation": f"（……予想以上の器量ね。あの状況で迷わず動けるなんて。もう少し、近くで観察させてもらうわ）",
        }
