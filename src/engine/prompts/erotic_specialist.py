...
class EroticSpecialist:
    ...
    def _get_erotic_config_value(self, yaml_preset: dict, key: str, default: Any) -> Any:
        """Get a value from the erotic YAML preset with fallback to default."""
        if yaml_preset and key in yaml_preset:
            return yaml_preset[key]
        return default
    ...
    def build_scene_prompt(
        ...
        # === Step 11 追加: YAMLプリセットがあれば優先使用 ===
        yaml_preset = context.get("erotic_rules_yaml")
        preset_name = yaml_preset.get("platform") if yaml_preset else context.get("platform_preset", "kakuyomu_romance")
        preset = get_preset(preset_name) if not yaml_preset else None
        # Use YAML preset values with fallback to preset file values
    ...
    def _build_narrative_flow_prompt(self, curve: EroticCurve) -> str:
        ...
        _ = [b for b in curve.beats if b.phase == "afterglow"]
