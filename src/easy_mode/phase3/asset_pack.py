...
class AssetPackGenerator:
    ...
    def generate_pack(
        ...
        **kwargs,
        ...
        if include_media_mix:
            media_files = self._generate_media_mix(series, subdirs["media_mix"], media_formats)
        ...
        # 4. 電子書籍生成
        if include_ebook:
            ebook_files = self._generate_ebooks(series, subdirs["ebook"], ebook_formats, **kwargs)
    ...
    def _save_original_novel(self, series: SeriesResult, output_dir: Path) -> Dict[str, str]:
        ...
        series_path.write_text(
            json.dumps(series_data, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        ...
        plot_path.write_text(
            json.dumps(series.plot_outline, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        ...
        bible_path.write_text(
            json.dumps(series.bible, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    ...
    def _generate_if_routes(self, series: SeriesResult, output_dir: Path) -> Dict[str, str]:
        ...
        for route_name, route_nodes in main_routes.items():
            ...
            route_file.write_text(
                json.dumps(
                    {"route_name": route_name, "nodes": [n.to_dict() for n in route_nodes]},
                    ensure_ascii=False,
                    indent=2,
                ),
                encoding="utf-8",
        ...
        save_template = {
            ...
            "save_slots": [],
            "settings": {"auto_save": True, "show_unavailable": False},
        }
        save_path = output_dir / "save_template.json"
        save_path.write_text(
            json.dumps(save_template, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        ...
        for node_id, node in graph.nodes.items():
            for choice in node.choices:
                if choice.target_node_id:
                    ...
                    lines.append(f'    "{node_id}" -> "{choice.target_node_id}" [label="{label}"];')
    ...
    def _generate_media_mix(
        ...
        for episode in series.episodes:
            ...
            scripts = self.media_exporter.export_all(episode, series, format_enums)
        ...
        index = {
            ...
            "episodes": {
                f"ep{ep.episode_num:03d}": {"title": ep.title, "scripts": list(media_formats)}
                for ep in series.episodes
            },
        }
    ...
    def _generate_promo_materials(self, series: SeriesResult, output_dir: Path) -> Dict[str, str]:
        ...
        (output_dir / "character_introductions.txt").write_text(char_intro, encoding="utf-8")
        files["character_introductions.txt"] = "キャラクター紹介"

        # キーワード・タグ
        keywords = self._generate_keywords(series)
        (output_dir / "keywords.txt").write_text(", ".join(keywords), encoding="utf-8")
        files["keywords.txt"] = "検索キーワード・タグ"

        # SNS用投稿文
        sns_posts = self._generate_sns_posts(series)
        (output_dir / "sns_posts.json").write_text(
            json.dumps(sns_posts, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    ...
    def _generate_synopsis(self, series: SeriesResult, long: bool = True) -> str:
        ...
        lines.append(
            f"全{series.total_episodes}話・約{sum(ep.word_count for ep in series.episodes):,}字"
        )
        lines.append("")
