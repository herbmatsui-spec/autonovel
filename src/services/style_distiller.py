...
NOTABLE_STYLE_SAMPLES: dict[str, dict] = {
    ...
    "style_dark_hero": {
        "name": "ダークヒーロー（ありふれ風）",
        "sample": "真紅の光が部屋に満ちる。敵は肉塊と化した。身内だけは、软件件名甘えられる。冷たく嘲う:「愚か者」
    },
    "style_overlord": {
        "name": "勘違い・魔王（オバロ風）",
...
class StyleDistillerService:
    ...
    def _calculate_rule_based_stats(
        self, text: str, name_hint: str | None = None
    ) -> StyleProfile:
        ...
        da_dearu_count = sum(1 for s in sentences if s.endswith(("だ", "である", "た", "いた", "った")))
        desu_masu_count = sum(1 for s in sentences if s.endswith(("です", "ます", "でした", "ました")))
        nominal_count = sum(1 for s in sentences if not s.endswith(("だ", "である", "た", "いた", "った", "です", "ます", "でした", "ました")))
