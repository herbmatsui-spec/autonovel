...
class LoadTestResult(BaseModel):
    ...
    def get_summary(self) -> Dict[str, Any]:
        return {
            ...
            "success_rate": self.episodes_completed
            / max(1, self.episodes_completed + self.episodes_failed),
...
class LoadTestMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "throughput_avg": sum(self.throughputs) / len(self.throughputs)
            if self.throughputs
            else 0,
...
class IntegrationLoadTester:
    ...
    async def _test_concurrent_episodes(self) -> LoadTestResult:
        ...
        try:
            ...
            return LoadTestResult(
                ...
                avg_quality_score=sum(r.get("quality", 0) for r in results if isinstance(r, dict))
                / max(1, episodes_completed),
            ...
            return LoadTestResult(
                ...
                episodes_failed=self.config.concurrent_episodes,
                total_characters=0,
    ...
    async def _test_cache_effectiveness(self) -> LoadTestResult:
        ...
        logger.info(
            f"Cache speedup: {cache_speedup:.2f}x (first: {time1:.0f}ms, second: {time2:.0f}ms)"
        )
    ...
    async def _test_resource_usage(self) -> LoadTestResult:
        ...
        # 負荷かけながらリソース使用率を監視
        tasks = [
            self._simulate_episode_generation(f"res_ep_{i}", f"res_content_{i}") for i in range(3)
        ]
    ...
    async def _test_quality_gate(self) -> LoadTestResult:
        ...
        test_cases = [
            {
                "content": "high quality content with good causality and proper structure",
                "expected_pass": True,
            },
    ...
    async def _test_end_to_end(self) -> LoadTestResult:
        ...
        # 実際にエピソードを生成してエ不通させる
        tasks = [
            self._simulate_episode_generation(f"e2e_ep_{i}", f"e2e_content_{i}")
            for i in range(self.config.concurrent_episodes)
        ]
        ...
        try:
            ...
            avg_quality = sum(r.get("quality", 0) for r in results if isinstance(r, dict)) / max(
                1, episodes_completed
            )
    ...
    async def _simulate_episode_generation(self, ep_id: str, content: str) -> Dict[str, Any]:
        ...
        return {
            "success": True,
            "chars": len(content) * 100,
            "quality": 0.8 + (hash(ep_id) % 20) / 100,
        }
    ...
    def _print_results(self, results: List[LoadTestResult]):
        ...
        for result in results:
            ...
            logger.info(
                f"  Episodes: {result.episodes_completed}/{result.episodes_completed + result.episodes_failed}"
            )
