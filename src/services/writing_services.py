...
class GenerationLoopManager:
    ...
    async def execute_generation_loop(
        self,
        ep_num: int,
        ctx: WritingContext,
        sys_inst: str,
        ...
        (
            ...
            ncs_score,
        ) = await self._phase_prepare_context(
        ...
        if reporter and max_ac_iter == 1:
            reporter.report(
                f"ℹ️ 第{ep_num}話: 通常回のため、Actor-Criticの反復回数を {max_ac_iter} に制限し、スパイクを物理的に防ぎます。",
                "info",
            )
        ...
        for ac_iter in range(max_ac_iter):
            ...
            logger.info(
                f"[Trace {trace_id}] Generating (AC Iter {ac_iter + 1}/{max_ac_iter}, Engine: {engine_key}, Temp: {temp:.2f})"
            )
            ...
            if not final_content:
                if fail_fast:
                    raise RuntimeError(
                        f"[Trace {trace_id}] Fail-Fast: Drafting yielded empty content."
                    )
                continue
        ...
        if should_heavy_audit and not is_causal_ok and not is_easy_mode:
            ...
            if not is_causal_ok and fail_fast:
                raise RuntimeError(
                    f"[Trace {trace_id}] Fail-Fast: Causality validation failed after healing. 指記矛盾: {causal_reason}"
                )
        ...
        if (is_integrity_ok and is_causal_ok and dogfeed_ok) or is_easy_mode:
            logger.info(
                f"[Trace {trace_id}] Ep.{ep_num} generation successful (Integrity: {rate:.2f}, Causality: {is_causal_ok})"
            )
        else:
            if fail_fast:
                raise RuntimeError(
                    f"[Trace {trace_id}] Fail-Fast: Post-generation loops failed. Integrity={is_integrity_ok}, Causal={is_causal_ok}, Dogfeed={dogfeed_ok}"
                )
            await self._register_lazy_patch(
                ep_num,
                ctx,
                is_integrity_ok,
                rate,
        ...
        try:
            ...
            immersion_score = next(
                (
                    s.get("score", 0.0)
                    for s in scored_scores
                    if s.get("metric_name") == "immersion_score"
                ),
                0.0,
            )
            if not is_easy_mode:
                ...
                if immersion_score < min_immersion and reporter:
                    reporter.report(
                        f"⚠️ 没入スコア {immersion_score:.1f} が閾値 {min_immersion} 未満です。",
                        "warning",
                    )
    ...
    async def _phase_prepare_context(
        self,
        ep_num: int,
        ctx: WritingContext,
        sys_inst: str,
    ...
    async def _phase_drafting(
        ...
        if should_beat_decompose:
            ...
            if reporter:
                reporter.report(
                    f"ℹ️ 第{ep_num}話: 標準エピソードのため、ビート分解をスキップしAPIコストを削減します（高速執筆）。",
                    "info",
                )
    ...
    async def _phase_audit(
        ...
        try:
            ...
            failures.append(
                {"rule": "確定ステータス不整合", "gap": str(e), "fragment": "ステータス検証エラー"}
            )
    ...
    async def _phase_critic(
        ...
        for f in failures:
            ...
            if severity_rank.get(sev, 0) < severity_rank.get(threshold, 3):
                ...
                if reporter:
                    reporter.report(
                        f"ℹ️ 第{ep_num}話 [Critic Iter {ac_iter + 1}]: 軽微な矛盾({sev})を検知しましたが、リライトをトリガーせず進行します。指摘: {f.get('gap', '')}",
                        "info",
                    )
                continue
            issues.append(
        ...
        if reporter:
            reporter.report(
                ...
                f"{len(issues)}件の矛盾を検知。修正指令を生成中...",
                "warning",
            )

        auditor = LogicalAuditor(repo=self.repo, llm=self.llm, pm=self.pm, ctx_mgr=None)
        ...
        if critic_fb.rewrite_guidance:
            ...
            if reporter:
                reporter.report(
                    f"🔄 第{ep_num}話 [Actor]: Criticの指示を受け、論理修正を加えてリライトします。",
                    "info",
                )
            return True
        return False

    def _determine_pov_instruction(
        self, ep_num: int, current_tension: int, is_catharsis: bool, reporter
    ) -> str:
        if current_tension >= 80 or is_catharsis:
            if reporter:
                reporter.report(
                    f"🔄 第{ep_num}話: 幕間・視点変更ロジック（敵の絶望/ヒロインの崇拝）を自動発動しました。",
                    "info",
                )
            return (
    ...
    def _calculate_ncs_score(self, ep_num: int, ctx: WritingContext) -> int:
        ...
        summary_text = (
            ...
            + (getattr(ctx.plot, "detailed_blueprint", "") or "")
        ).lower()
        ncs_score = 0
        if getattr(ctx.plot, "is_catharsis", False):
            ncs_score += 50
        ...
        if beats_list:
            ...
            for b in beats_list:
                ...
                formatted_beats.append(
                    f"ビート{num}: {action} [五感: {sensory}, フェーズ: {phase}, 目標文字数: {budget}字]"
                )
            if reporter:
                reporter.report(
                    f"✅ 第{ep_num}話: {len(beats_list)}個のビート分解完了。描写の解像度を最大化します。",
                    "success",
                )
            return "\n".join(formatted_beats)
        return beat_content or ""

    async def _draft_episode_parts(
        self, ep_num: int, gen_ctx: WritingGenerationContext, temp: float, reporter
    ) -> str:
        ...
        p1_prompt = gen_ctx.build_fw_prompt(
        ...
        p2_prompt = gen_ctx.build_fw_prompt(
    ...
    async def _polishing_pass(
        ...
        if reporter:
            reporter.report(
                f"✨ 第{ep_num}話: 研磨パス（五感・禁止語・リズム・目標文字数）を実行中...", "info"
            )
        ...
        if (
            ...
            and len(res.story_content.strip()) > len(draft_content) * min_ratio
        ):
            if reporter:
                reporter.report(
                    f"✨ 第{ep_num}話: 研磨が完了しました。({len(draft_content)}字 -> {len(res.story_content)}字)",
                    "success",
                )
        ...
        if reporter:
            reporter.report(
                f"⚠️ 第{ep_num}話: 研磨パスに失敗、または出力が極端に短いため初稿をフォールバックとして採用します。",
                "warning",
            )
    ...
    async def _run_causality_audits(
        self,
        ep_num: int,
        ctx: WritingContext,
        content: str,
        ...
        if should_heavy_audit and monitor and any(kw in content for kw in AUDIT_TRIGGER_KEYWORDS):
            ...
            is_causal_ok, causal_reason, failures = await monitor.audit_setting_causality(
                content, world_settings_str, blueprint
            )

            if active_constraints:
                tests_ok, test_failures = await monitor.run_constraint_unit_tests(
    ...
    async def _apply_surgical_healing(
        self,
        ep_num: int,
        content: str,
        ctx: WritingContext,
        ...
        snippets = [
            f.get("fragment", "")
            for f in failures
            if f.get("fragment") and f.get("fragment") != "..."
        ]
        ...
        healed_content = await self.surgical_causality_healing_pass(
            content, world_settings_str, blueprint, causal_reason, snippets
        )

        if healed_content:
            if active_constraints:
                recheck_ok, recheck_failures = await monitor.run_constraint_unit_tests(
                    healed_content, active_constraints
                )
                if recheck_ok:
                    return healed_content, True, ""
                else:
                    logger.warning(
                        f"Surgical healing failed re-audit. {len(recheck_failures)} remaining."
                    )
    ...
    async def _run_dogfeeding_loop(
        self,
        ep_num: int,
        content: str,
        passion: float,
        ...
        if should_dogfeed and attempt < max_retries - 1:
            eval_result = await self.critique.run_dogfeeding_approval_loop(
                content, ep_num, passion, temp
            )
            score = eval_result.get("score", 100)
            if score < 80:
                if reporter:
                    reporter.report(
                        f"⚠️ 第{ep_num}話: 自己評価スコアが低いため({score}点)再生成します。理由: {eval_result.get('reason', '')}",
                        "warning",
                    )
            ...
            reporter.report(
                f"ℹ️ 第{ep_num}話: 重要度が低いため自己評価(Dogfeeding)をスキップし高速化しました。",
                "info",
            )
        return True

    async def _register_lazy_patch(
        self,
        ep_num: int,
        ctx: WritingContext,
        ...
        try:
            ...
            if latest_bible_model:
                ...
                await self.repo.create_bible(
                    book_id, settings_dict, new_version, time.strftime("%Y-%m-%dT%H:%M:%S")
                )

                if reporter:
                    reporter.report(
                        f"🩹 第{ep_num}話: 矛盾を完全に解消できませんでした。次話以降で自動回収・言い訳を行う「遅延パッチ」をWorldBibleに登録し、強制進行します。",
                        "warning",
                    )
            else:
    ...
    async def surgical_causality_healing_pass(
        self,
        content: str,
        world_settings: str,
        blueprint: str,
